...
public:
	...
	bool initialize();
	void runLoop();
	const string getFilename(const char *name, const char *fileExtension,
		bool replaceSpaces = true, bool replaceUnderscores = false);
	void shutdown();
	void quit();
	...
	void broadcastServerConnected(const unsigned char &iID,
		const SystemAddress &exceptionAdd);
	void broadcastServerDisconnected(const unsigned char &iID,
		const SystemAddress &exceptionAdd);
	void broadcastMaintenance(const unsigned char &time);
	const OwnerToken assignToken(const SystemAddress &add,
		const unsigned char &serverID, const string &name,
		const unsigned short &charID);
