...
struct WorldCritter
{
    ...
    WorldCritter(const String &t="", const Real &sR=0, bool hRA=false,
        const unsigned short &rAID=0)
    {
        type = t;
        spawnRate = sR;
        hasRoamArea = hRA;
...
public:
    ...
    void loadSettings(bool &isTablet, Real &mouseSensitivity, Real &musicVol, Real &guiVol,
        Real &soundVol, bool &isShadowsOn, bool &isBloom, String &username,
        bool &generalCharname, bool &localUsername, bool &channelBlink, bool &runMode,
        bool &doubleJumpOn, bool &autoLipSync, bool &useWindowsCursor);
    void saveSettings(bool isTablet, const Real &mouseSensitivity, const Real &musicVol,
        const Real &guiVol, const Real &soundVol, const bool &isShadowsOn, bool isBloom,
        const String &username = "", bool generalCharname = false, bool localUsername = false,
        bool channelBlink = true, bool runMode = true, bool doubleJumpOn = true,
        bool autoLipSync = false, bool useWindowsCursor = false);
    ...
    bool loadWorld(const String &filename, String &terrain, Real &x, Real &z,
        Vector3 &spawnSquare, Vector2 &worldBounds);
    ...
    bool loadCampaign(const String &name, CampaignEventList &data, String &fileName,
        bool customCampaigns = false);
    void loadCampaignScript(const String &filename, const vector<String>::type &nextEvent,
        CampaignEventList &data, const unsigned short &eventCount = 0,
        bool loadFirstSection = false);
    ...
    bool loadHomePoint(String &mapName, Vector2 &point, unsigned char &dimension,
        bool isSavePoint = false);
    void saveSavePoint(const String &mapName, const Vector2 &point,
        const unsigned char &dimension);
    void saveHomePoint(const String &mapName, const Vector2 &point,
        const unsigned char &dimension, bool isSavePoint = false);
    ...
    void loadWeatherCycle(const String &type, vector<WeatherEvent>::type &list,
        bool isCustom = false);
    void loadWeather(const String &type, String &particle, Vector3 &offset, String &sound,
        bool isCustom = false);
    ...
    const std::pair<CritterAttack, String> getCritterAttackAndSound(const String &type,
    ...
    bool loadCritterSpawnListFile(const String &filename, const String &worldName,
        unsigned short &limit, vector<WorldCritter>::type &list,
        vector<std::pair<Vector3, Vector3>>::type &roamArea);
    bool loadCustomCritterSpawnList(const String &filename, unsigned short &limit,
        vector<WorldCritter>::type &list,
        vector<std::pair<Vector3, Vector3>>::type &roamArea);
