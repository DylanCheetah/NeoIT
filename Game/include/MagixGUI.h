...
public:
    ...
    void initialize(SceneManager *sceneMgr, RenderWindow *window, MagixExternalDefinitions *def, 
        MagixGameStateManager *gameStateMgr, MagixItemManager *itemManager, 
        MagixCritterManager *critterMgr, MagixUnitManager *unitManager, 
        MagixNetworkManager *networkManager, MagixCamera *camera, 
        MagixSoundManager *soundMgr, MagixEffectsManager *effectsMgr, 
        MagixCollisionManager *collisionMgr);
    void showFaderOverlay(bool flag);
    void showPseudoShaderOverlay(bool flag);
    void showCursorOverlay(bool flag);
    void showBackdropOverlay(bool flag, bool showLoadingText = false, 
        const String &loadingCaption = "Loading...");
    ...
    void showCreditsOverlay(bool flag, bool showCreditsCaption = true, 
        bool showGameOverButtons = false);
    ...
    void toggleInputMode(bool isEnter, OverlayElement *inputBox = 0, 
        String defaultCaption = "Say: ", unsigned short charLimit = 256, bool clearAtReturn = true, 
        String startText = "", bool allowTempStorage = true, bool allowNewLine = false, 
        bool hideText = false);
    bool selectCurrentObject(SceneNode *object, const unsigned int &flags = 0, 
    ...
    void sizeWindowDrag(unsigned short sizeDir, const Real &minWidth = 0, 
        const Real &minHeight = 0);
    ...
    void updateBoxScrollerCaption(const unsigned short &boxID, const unsigned short &scrollerID, 
        const vector<String>::type &captionList);
    const short getListButtonLine(const unsigned short &boxID, const unsigned short &scrollerID, 
        const unsigned short &listLines, Real &fontHeight, short &apparentLine);
    const short getListButtonLine(const unsigned short &boxID, Real &fontHeight, 
        short &apparentLine);
    ...
    bool checkItemEquippable(const String &meshName, bool isCreate = false, bool doEquip = false, 
        bool ground = false);
