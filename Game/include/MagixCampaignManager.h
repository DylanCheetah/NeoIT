...
struct ScriptedUnit
{
    ...
    vector<vector<String>::type>::type eventConditionList;
...
public:
    ...
    void initialize(MagixExternalDefinitions *def, MagixGameStateManager *gameStateMgr,
        MagixGUI *gui, MagixUnitManager *unitMgr, MagixWorld *world, MagixSkyManager *skyMgr,
        MagixCamera *camera, MagixSoundManager *soundMgr, MagixCollisionManager *collisionMgr,
    ...
    void closeCampaign(unsigned short restartState = GAMESTATE_INITIALIZE,
        const String &creditsName = "");
    void updateCredits(const FrameEvent &evt);
    bool compare(String first, String second);
    void processEvent(const CampaignEvent &cEvent, bool isInitialize = false,
        ScriptedUnit *sUnit = 0);
