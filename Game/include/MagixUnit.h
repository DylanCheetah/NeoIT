...
public:
    ...
    MagixUnit();
    ~MagixUnit();
    void createUnit(const unsigned short iID, SceneManager *sceneMgr, const String &bodyMesh,
        const String &headMesh, const String &maneMesh, const String &tailMesh,
        const String &wingMesh, const String &matName, const String &tuftMesh);
    void destroyUnit(SceneManager *sceneMgr);
    void setMaterial(const String &matName);
    void setColours(const ColourValue &pelt, const ColourValue &underfur, const ColourValue &mane,
        const ColourValue &nose, const ColourValue &eyeTop, const ColourValue &eyeBot,
        const ColourValue &ears, const ColourValue &tailtip, const ColourValue &eyes,
        const ColourValue &wing, const ColourValue &marking);
    ...
    void setTarget(const Real &x, const Real &y, const Real &z,
        const unsigned char &action = TARGET_RUNTO);
    ...
    void changeAnimBase(const String &type, const Real &speed, bool resetTimePosition = false,
        bool reverseAnimation = false, bool loop = true);
    void changeAnimSecond(const String &type, const Real &speed, bool resetTimePosition = false,
        bool reverseAnimation = false, bool loop = true);
    ...
    void createChatBubble(String caption, MovableTextOverlayAttributes *attrs,
        const Real &tLeft = 0.0, const Real &tTop = 0.0);
    ...
    void replaceBodyMesh(SceneManager *sceneMgr, const String &meshName,
        bool resetMaterial = false);
    void replaceHeadMesh(SceneManager *sceneMgr, const String &meshName,
        bool resetMaterial = false);
    void replaceManeMesh(SceneManager *sceneMgr, const String &meshName,
        bool resetMaterial = false);
    void replaceTailMesh(SceneManager *sceneMgr, const String &meshName,
        bool resetMaterial = false);
    void replaceWingMesh(SceneManager *sceneMgr, const String &meshName,
        bool resetMaterial = false);
    void replaceTuftMesh(SceneManager *sceneMgr, const String &meshName,
        bool resetMaterial = false);
    ...
    void setCustomAnimation(bool flag, const String &animName = "", Real animSpeed = 1,
        bool loop = false, bool reverse = false);
    virtual unsigned short getControlMode();
    void setIsSwimming(bool flag);
    bool getIsSwimming();
    void equip(SceneManager *sceneMgr, MagixExternalDefinitions *mDef, const String &meshName,
        const Vector3 &offset = Vector3::ZERO, const short &slot = -1, bool altAnim = false);
    const String unequip(SceneManager *sceneMgr, MagixExternalDefinitions *mDef,
    ...
    void setMarkings(const unsigned char &bodyID, const unsigned char &headID,
