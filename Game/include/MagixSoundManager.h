...
protected:
    ...
    MagixCamera *mCamera;
    int music;
    FMOD_CHANNEL*mMusicChannel;
    bool loopMusic;
    Real musicFadeTimeout;
    int guiSound;
    FMOD_CHANNEL*mGuiChannel;
    int ambientSound;
    FMOD_CHANNEL*mAmbientChannel;
    float musicVolume;
    float guiVolume;
    float soundVolume;
...
public:
    ...
    void playMusic(String type, bool loop = true);
    void _playMusic(String type, bool loop = true);
    ...
    void playLoopedSound(const char *type, SceneNode *node, const float &volume = 1,
        const Real &delay = 0);
    const list<LoopedSound>::type::iterator stopLoopedSound(
