...
public:
    ...
    void createRibbonTrail(Entity *ent, const String &boneName, const String &trailMat, 
        const Real &trailWidth = 2,
        const ColourValue &trailColour = ColourValue(1, 0.9, 0.75, 1), 
        const ColourValue &colourChange = ColourValue(7, 7, 7, 7), const String &headMat = "");
    void destroyRibbonTrail(unsigned short tID);
    ParticleSystem* createParticle(SceneNode *node, const String &name, const Real &duration, 
        bool unresettable = false, bool attachToBone = false, bool hasOwnNode = false, 
        const Vector3 &position = Vector3::ZERO, const String &ownName = "");
    ...
    void createItemEffect(Entity *owner, const String &type, const String &bone, 
