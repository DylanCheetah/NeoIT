...
public:
    ...
    void initialize(SceneManager *sceneMgr, MagixExternalDefinitions *def,
        MagixGameStateManager *gameStateMgr, MagixEffectsManager *effectsMgr,
        MagixCollisionManager *collMgr, MagixSoundManager *soundMgr, MagixWorld *world,
        MagixCamera *camera);
    void reset();
    ...
    void hitCritter(MagixCritter *critter, const Real &damage, const Vector3 &force,
        const OwnerToken &token = 0, bool inRange = false);
    void hitCritter(const unsigned short &iID, const Real &damage, const Vector3 &force,
        const OwnerToken &token);
    void updateGroundHeight(MagixCritter *critter, bool checkWaterCollision = false,
        bool checkCollBoxCollision = false);
    MagixCritter* createCritter(const unsigned short &iID, const Critter &critter,
        const Vector3 &position, const short &owner = -1);
    MagixCritter* createCritter(const unsigned short &iID, const Critter &critter,
        const Vector3 &position, const unsigned char &worldID, const short &owner = -1);
