...
public:
    ...
    void updateMovement(const FrameEvent &evt,
        const Quaternion &camOrientation = Quaternion::ZERO);
    ...
    void update(const FrameEvent &evt, MagixExternalDefinitions *def,
        const Quaternion &camOrientation = Quaternion::ZERO);
    const OBJECT_TYPE getType();
    void toggleControlMode();
    void setTarget(const Vector3 &dest, const unsigned char &action = TARGET_RUNTO,
        bool pickupPlus = false);
