...
public:
    ...
    */
    ...
    Ogre::DataStreamPtr resourceLoading(const String &name, const String &group,
        Resource *resource);
    bool resourceCollision(ResourcePtr &resource, ResourceManager *resourceManager);

    //Custom callbacks of sorts
    void matGenerationStarted(const String& groupName, size_t scriptCount);
    void matGenerateStarted(const String& scriptName, bool &skipThisScript);
    void matGenerateEnded(const String& scriptName, bool skipped);
};
