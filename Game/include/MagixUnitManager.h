...
public:
    MagixUnitManager();
    ~MagixUnitManager();
    void initialize(SceneManager *sceneMgr, MagixExternalDefinitions *def,
        MagixGameStateManager *gameStateMgr, MagixWorld *world,
        MagixEffectsManager *effectsMgr, MagixCollisionManager *collisionMgr,
        MagixSoundManager *soundMgr, MagixItemManager *itemMgr,
        MagixCritterManager *critterMgr, MagixCamera *camera);
    ...
    void setMapChange(bool changeMap, const String &mapName, bool changePoint = false,
        const Vector3 &point = Vector3::ZERO, bool hasVectY = false);
    ...
    const Real getGroundHeight(const Real &x, const Real &z, bool checkWaterCollision = false,
        bool checkCollBoxCollision = false);
    MagixPlayer* getPlayer();
    MagixIndexedUnit* createUnit(const unsigned short iIndex, const String &bodyMesh,
    ...
    void createPlayerUnit(const String &bodyMesh, const String &headMesh, const String &maneMesh,
    ...
    ParticleSystem* createParticleOnUnit(MagixUnit *unit, const String &particleName,
        const Real &duration, bool unresettable = false);
    ParticleSystem* createParticleOnUnitBone(MagixUnit *unit, const String &tBone,
        const String &particleName, const Real &duration, bool unresettable = false);
    ...
    void createItem(const unsigned short &iID, const String &meshName, const Real &tX,
        const Real &tZ);
    void deleteItem(const unsigned short &iID);
    void deleteItem(MagixItem *item);
    MagixCritter* createCritter(const unsigned short &iID, const unsigned char &worldID,
        const Vector3 &position, const short &owner = -1);
    MagixCritter* createCritter(const unsigned short &iID, const String &type,
        const Vector3 &position, const short &owner = -1);
