...
void MagixLoadingBar::start(RenderWindow* window, unsigned short numGroupsInit, unsigned short numGroupsLoad, Real initProportion)
{
    ...
    mLoadingBarElement = omgr.getOverlayElement("Core/LoadingScreen/LoadPanel/Bar/Progress");
    mLoadingCommentElement = omgr.getOverlayElement("Core/LoadingScreen/LoadPanel/Comment");
    mLoadingDescriptionElement = omgr.getOverlayElement(
        "Core/LoadingScreen/LoadPanel/Description");

    OverlayElement* barContainer = omgr.getOverlayElement("Core/LoadingScreen/LoadPanel/Bar");
...
void MagixLoadingBar::resourceGroupScriptingStarted(const String& groupName,
    size_t scriptCount)
{
    ...
    mLoadingDescriptionElement->setCaption("Loading game...");
    mWindow->update();
}


void MagixLoadingBar::scriptParseStarted(const String& scriptName,
    bool &skipThisScript)
{
...
void MagixLoadingBar::resourceGroupLoadStarted(const String& groupName,
...
Ogre::DataStreamPtr MagixLoadingBar::resourceLoading(const String &name,
    const String &group, Resource *resource)
{
    return Ogre::DataStreamPtr();
}


bool MagixLoadingBar::resourceCollision(ResourcePtr &resource,
...
void MagixLoadingBar::matGenerationStarted(const String& groupName,
    size_t scriptCount)
{
    assert(mNumGroupsInit > 0 && "You stated you were not going to init "
        "any groups, but you did! Divide by zero would follow...");
    ...
    mProgressBarInc = mProgressBarMaxSize * mInitProportion / (Real)scriptCount;
    mProgressBarInc /= mNumGroupsInit;
    mLoadingDescriptionElement->setCaption("Generating materials...");
    mWindow->update();
}


void MagixLoadingBar::matGenerateStarted(const String& scriptName,
    bool &skipThisScript)
{
    mLoadingCommentElement->setCaption(scriptName);
    mWindow->update();
...
void MagixLoadingBar::matGenerateEnded(const String& scriptName, bool skipped)
{
    mLoadingBarElement->setWidth(
        mLoadingBarElement->getWidth() + mProgressBarInc);
    mWindow->update();
