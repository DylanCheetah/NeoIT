...
MagixNetworkManager::MagixNetworkManager()
{
    ...
    lastMovePosition = std::pair<Vector3, Real>(Vector3::ZERO, (Real)0);
...
void MagixNetworkManager::initialize(MagixExternalDefinitions *def,
    MagixGameStateManager *gamestateMgr, MagixItemManager *itemMgr,
    MagixCritterManager *critterMgr, MagixUnitManager *unitMgr,
    MagixWorld *world, MagixSkyManager *skyMgr, MagixChatManager *chatMgr,
    MagixAlertBox *alertBox, MagixCharScreenManager *charscreenMgr)
{
    mDef = def;
...
bool MagixNetworkManager::connect()
{
    ...
    mPeer->Startup(1, &tmp, 1, 30);

    const String tLocalIP = mDef->loadLocalIP("localIP.txt");

    if(tLocalIP != "")
    {
        mPeer->Connect(tLocalIP.c_str(), SERVER_PORT, SERVER_PASSWORD,
            (int)strlen(SERVER_PASSWORD));
        mChatManager->message("Connecting to Server " + tLocalIP);
    }
    else
    {
        unsigned char tServerID = (unsigned char)Math::RangeRandom(0,
            (Real)serverAdd.size());

        if(tServerID >= (int)serverAdd.size())
        {
            tServerID = (int)serverAdd.size() - 1;
        }

        mPeer->Connect(serverAdd[tServerID].second.ToString(false),
            serverAdd[tServerID].second.GetPort(), SERVER_PASSWORD,
            (int)strlen(SERVER_PASSWORD));
        mChatManager->message(
            "Connecting to Server " + StringConverter::toString(
            serverAdd[tServerID].first + 1));
    }
...
void MagixNetworkManager::connectMainServer()
{
    ...
    if(tLocalIP != "")
    {
        mPeer->Connect(tLocalIP.c_str(), MAIN_SERVER_PORT, SERVER_PASSWORD,
            (int)strlen(SERVER_PASSWORD));
    }
    else
    {
        mPeer->Connect(MAIN_SERVER_IP, MAIN_SERVER_PORT, SERVER_PASSWORD,
            (int)strlen(SERVER_PASSWORD));
    }
...
void MagixNetworkManager::disconnect()
{
    mPeer->Shutdown(300);
    isMainServerConnection = false;
    acceptedFlag = false;
...
void MagixNetworkManager::reset()
{
    playerPositionQueue.clear();
    lastMovePosition = std::pair<Vector3, Real>(Vector3::ZERO, (Real)0);
    lastFaceDirection = Degree(0);
...
void MagixNetworkManager::update(const FrameEvent evt)
{
    if(acceptedFlag)
    {
        ...
        {
            moveUpdateCount += (Real)(mUnitManager->getPlayer()->getIsCrouching() ?
                0.75 : (mUnitManager->getPlayer()->getIsWalking() ? 0.5 : 0.25));
            sendMovePosition();
            sendFaceDirection();
        }
    ...
    while(packet && tCont)
    {
        ...
        case ID_CONNECTION_REQUEST_ACCEPTED:
            ...
            {
                networkMessage = NetworkMessage(NETWORK_CONNECTED);
            }
            else
            {
        ...
        case ID_CONNECTION_ATTEMPT_FAILED:
            mAlertBox->showAlert("Our connection request has failed :c", 0.5,
                0.5, 10);

            if(isMainServerConnection)
            {
                networkMessage = NetworkMessage(NETWORK_CONNECTIONFAILED);
            }
            else
            {
                mGameStateManager->setGameState(GAMESTATE_RETURNSTARTSCREEN);
        ...
        case ID_INVALID_PASSWORD:
            mAlertBox->showAlert("Please download the latest version c:", 0.5,
                0.5, 10);
        ...
        case ID_NO_FREE_INCOMING_CONNECTIONS:
            mAlertBox->showAlert("This server is temporarily unavailable :B",
                0.5, 0.5, 10);
        ...
        case ID_DISCONNECTION_NOTIFICATION:
            if(isMainServerConnection)
            {
                mAlertBox->showAlert("We have been disconnected :c", 0.5, 0.5,
                    10);
                networkMessage = NetworkMessage(NETWORK_LOSTCONNECTION);
            }
            else
            {
                ...
                for(vector<std::pair<unsigned char, SystemAddress>>::type::iterator it = serverAdd.begin();
                    it != serverAdd.end(); it++)
                {
                ...
                mAlertBox->showAlert("Disconnected, reconnecting...", 0.5, 0.5,
                    2.5);
                mChatManager->message(
                    "Disconnected from Server " + StringConverter::toString(
                    tServerID + 1) + ".");
                disconnect();
                mItemManager->reset();
        ...
        case ID_CONNECTION_LOST:
            ...
            else
            {
                ...
                mAlertBox->showAlert("Connection lost, reconnecting...", 0.5,
                    0.5, 2.5);
                mChatManager->message(
                    "Connection lost from Server " + StringConverter::toString(
                    tServerID + 1) + ", reconnecting...");
                disconnect();
                mItemManager->reset();
                mCritterManager->reset();
                mUnitManager->reset(false, false);
        ...
        case ID_NEWPLAYERUPDATE:
            processAndSendNewPlayerUpdate(packet);
            break;

        case ID_REQUESTMAPINFO:
            processAndSendMapInfo(packet);
            break;
        ...
        case ID_MOVEPOSITION:
            processMovePosition(packet);
            break;

        case ID_LOOKQUATERNION:
            processLookQuaternion(packet);
            break;

        case ID_FACEDIRECTION:
            processFaceDirection(packet);
            break;

        case ID_ACTIONFLAGS:
            processActionFlags(packet);
            break;
        ...
        case ID_LOADCHAR:
            processLoadChar(packet);
            break;

        case ID_CREATECHAR:
            processCreateChar(packet);
            break;

        case ID_EDITCHAR:
            processEditChar(packet);
            break;

        case ID_EDITACCOUNT:
            processEditAccount(packet);
            break;

        case ID_KICK:
            tCont = processKick(packet);
            break;

        case ID_ITEMDROP:
            processItemDrop(packet);
            break;

        case ID_ITEMPICKUP:
            processItemPickup(packet);
            break;

        case ID_ITEMEQUIP:
            processItemEquip(packet);
            break;

        case ID_ITEMUNEQUIP:
            processItemUnequip(packet);
            break;

        case ID_GODSPEAK:
            processGodSpeak(packet);
            break;

        case ID_FORCELOGOUT:
            tCont = processForceLogout(packet);
            break;
        ...
        case ID_CRITTERTARGETEDINFO:
            processCritterTargetedInfo(packet);
            break;
        ...
        case ID_ITEMSTASH:
            processItemStash(packet);
            break;

        case ID_FINDPLAYER:
            processFindPlayer(packet);
            break;
        ...
        case ID_SERVERCONNECTED:
            processServerConnected(packet);
            break;

        case ID_MAINTENANCE:
            processMaintenance(packet);
            break;
...
const MessageID MagixNetworkManager::getPacketIdentifier(Packet *p)
{
    if(p == 0)
    {
        return 255;
...
const String MagixNetworkManager::getPlayerMap(String name)
{
    ...
    for(vector<PlayerData>::type::iterator it = playerList.begin(); it != playerList.end(); it++)
    {
        ...
        if(tName == name)
        {
            return mDef->getMapName(tData->mapName);
        }
    }
...
void MagixNetworkManager::processPlayerToken(Packet *p)
{
...
void MagixNetworkManager::sendPlayerTokenRequest()
{
    RakNet::BitStream tBitStream;

    tBitStream.Write(MessageID(ID_TOKENCONNECTED));
    stringCompressor->EncodeString(mUnitManager->getPlayer()->getUser().c_str(), 16,
        &tBitStream);
    tBitStream.Write((unsigned short)mCharScreenManager->getCharID());

    mPeer->Send(&tBitStream, HIGH_PRIORITY, RELIABLE, 0, UNASSIGNED_SYSTEM_ADDRESS,
        true);
}


void MagixNetworkManager::processChat(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    else
    {
        ...
        if(tLipSync || tType == CHAT_LIPSYNC)
        {
            tUnit->doLipSync(tCaption);
        }

        if(tType != CHAT_LIPSYNC)
        {
            mChatManager->say(mUnitManager, tUnit, String(tCaption), tType);
        }
    }
}


void MagixNetworkManager::sendChat(const String &caption, const unsigned char &type,
    const String &pmTarget, bool autoLipSync)
{
    RakNet::BitStream tBitStream;
    ...
    mPeer->Send(&tBitStream, MEDIUM_PRIORITY, RELIABLE, 0, UNASSIGNED_SYSTEM_ADDRESS,
        true);
}


void MagixNetworkManager::sendPartyChat(const OwnerToken &target, const String &caption,
    const unsigned char &type, bool autoLipSync)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processNewPlayer(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    if(mUnitManager->getUnitByIndex(tToken))
    {
        return;
    }
    ...
    if(String(tMatName) == "CustomMat")
    {
        ...
        tUnit->setMarkings((unsigned char)(mDef->hasBodyMark((unsigned char)tBodyMark) ?
        tBodyMark : 0),
                    (unsigned char)(mDef->hasHeadMark((unsigned char)tHeadMark) ? tHeadMark : 0),
                    (unsigned char)(mDef->hasTailMark((unsigned char)tTailMark) ? tTailMark : 0));
    ...
    if(tUnit->isAdmin)
    {
        ...
        mUnitManager->createAdminAura(tUnit, StringConverter::parseColourValue(String(
            tAdminColour)));
    }
    ...
    mChatManager->message(tAddName + tAddMessage + " has arrived.",
        (mUnitManager->isPartyMember(tUnit) ? 0 : (mChatManager->getChannel() == 0 ?
    CHAT_LOCAL : CHAT_GENERAL)));

    if(tBadData)
    {
        mChatManager->message("Error notice:" + tAddName + "has sent erroneous data.");
    }
    ...
    for(vector<std::pair<OwnerToken, PositionInfo>>::type::iterator it = playerPositionQueue.begin(); it != playerPositionQueue.end(); it++)
    {
        ...
        if(tPos.first == tToken)
        {
            ...
            if(tPos.second.hasOwnGroundHeight)
            {
                tUnit->setGroundHeight(tPos.second.groundHeight);
            }
            else
            {
                mUnitManager->clampUnitToTerrain(tUnit, false);
            }
...
void MagixNetworkManager::sendNewPlayer(const OwnerToken &target)
{
    RakNet::BitStream tBitStream;
    ...
    stringCompressor->EncodeString(mUnitManager->getPlayer()->getUser().c_str(), 16,
        &tBitStream);
    stringCompressor->EncodeString(mUnitManager->getPlayer()->getName().c_str(), 32,
        &tBitStream);
    stringCompressor->EncodeString(mUnitManager->getPlayer()->getMeshString().c_str(), 128,
        &tBitStream);
    stringCompressor->EncodeString(mUnitManager->getPlayer()->getMatGroupName().c_str(), 32,
        &tBitStream);
    stringCompressor->EncodeString(mUnitManager->getPlayer()->getMeshString2().c_str(), 32,
        &tBitStream);
    ...
    if(!tDefaultScale)
    {
        tBitStream.Write(mUnitManager->getPlayer()->getObjectNode()->getScale().x);
    }
    ...
    if(mUnitManager->getPlayer()->getMatGroupName() == "CustomMat")
    {
        ...
        stringCompressor->EncodeString(mUnitManager->getPlayer()->getColourString().c_str(), 256,
            &tBitStream);
    }

    stringCompressor->EncodeString(mDef->isAdmin ? "ok." : (mDef->isMod ? "mod" : "nil"), 8,
        &tBitStream);

    if(mDef->isAdmin)
    {
        stringCompressor->EncodeString(StringConverter::toString(mDef->adminAuraColour).c_str(),
            16, &tBitStream);
    }
...
void MagixNetworkManager::processPosition(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    if(!tUnit)
    {
        playerPositionQueue.push_back(std::pair<OwnerToken, PositionInfo>(tToken,
            PositionInfo(tPosition, tHasGroundHeight, tGroundHeight)));
        return;
    }
...
void MagixNetworkManager::sendPosition(const OwnerToken &target)
{
    RakNet::BitStream tBitStream;
    ...
    if(tOwnGroundHeight)
    {
        tBitStream.Write(mUnitManager->getPlayer()->getGroundHeight());
    }
...
void MagixNetworkManager::processMovePosition(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
...
void MagixNetworkManager::sendMovePosition()
{
    ...
    if(tOwnGroundHeight)
    {
        tBitStream.Write(tHeight);
    }

    mPeer->Send(&tBitStream, LOW_PRIORITY, UNRELIABLE_SEQUENCED, 5,
        UNASSIGNED_SYSTEM_ADDRESS, true);
}


void MagixNetworkManager::processLookQuaternion(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
...
void MagixNetworkManager::sendLookQuaternion(const OwnerToken &target)
{
    const bool tFreeLook = mUnitManager->getPlayer()->getFreeLook();

    if(target == TOKEN_NULL)
    {
        if(!tFreeLook && !lastLookQuaternion.first)
        {
            return;
        }
    ...
    if(lastLookQuaternion.second == tDir)
    {
        return;
    }
...
void MagixNetworkManager::processFaceDirection(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    if(tUnit->isDisabled() && tUnit->getHP() <= 0)
    {
        return;
    }
...
void MagixNetworkManager::sendFaceDirection(const OwnerToken &target)
{
    const Degree tYaw = mUnitManager->getPlayer()->getYaw();

    if(target == TOKEN_NULL)
    {
        if(lastFaceDirection == tYaw)
        {
            return;
        }
...
void MagixNetworkManager::processActionFlags(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    if(tHasFlags)
    {
        ...
        unsigned char tFlags = (unsigned char)tData.to_ulong();
        ...
        if((tFlags == 4) != tUnit->getIsCrouching())
        {
            tUnit->doCrouch(tFlags == 4);
        }

        tUnit->setIsWalking(tFlags == 5);

        if((tFlags == 6) != tUnit->getIsStance())
        {
            tUnit->toggleStance(mDef);
        }
    }
    else
    {
        ...
        if(tUnit->getIsCrouching())
        {
            tUnit->doCrouch(false);
        }

        tUnit->setIsWalking(false);

        if(tUnit->getIsStance())
        {
            tUnit->toggleStance(mDef);
        }
    }
    ...
    if(tHasPrimary)
    {
        ...
        unsigned char tPrimary = (unsigned char)tData.to_ulong();
    ...
    if(tHasSecondary)
    {
        ...
        unsigned char tSecondary = (unsigned char)tData.to_ulong();

        if(tSecondary == 1)
        {
            tUnit->doNodHead();
        }
        else if(tSecondary == 2)
        {
            tUnit->doShakeHead();
        }
        else if(tSecondary == 3)
        {
            tUnit->doNodHeadSlow();
        }
        else if(tSecondary == 4)
        {
            tUnit->doShakeHeadSlow();
        }
        else if(tSecondary == 5)
        {
            tUnit->doHeadTilt();
        }
        else if(tSecondary == 8)
        {
            tUnit->doSniff();
        }
        else if(tSecondary == 9)
        {
            tUnit->doTailFlick();
        }
        else if(tSecondary == 10)
        {
            tUnit->doLaugh();
        }
        else if(tSecondary == 11)
        {
            tUnit->doChuckle();
        }

        if((tSecondary == 6) != tUnit->isLick())
        {
            tUnit->toggleLick();
        }

        if((tSecondary == 7) != tUnit->isNuzzle())
        {
            tUnit->toggleNuzzle();
        }
    }
...
void MagixNetworkManager::sendActionFlags(const OwnerToken &target)
{
    //Flags = 6
    unsigned char tFlags = 0;

    if(mUnitManager->getPlayer()->isAntiGravity())
    {
        tFlags = 1;
    }
    else if(mUnitManager->getPlayer()->getHasDoubleJumped())
    {
        tFlags = 2;
    }
    else if(mUnitManager->getPlayer()->getJump())
    {
        tFlags = 3;
    }
    else if(mUnitManager->getPlayer()->getIsCrouching())
    {
        tFlags = 4;
    }
    else if(mUnitManager->getPlayer()->getIsWalking())
    {
        tFlags = 5;
    }
    else if(mUnitManager->getPlayer()->getIsStance())
    {
        tFlags = 6;
    }
    ...
    else if(mUnitManager->getPlayer()->isLaying())
    {
        tPrimary = 2;
    }
    else if(mUnitManager->getPlayer()->isPlopped())
    {
        tPrimary = 3;
    }
    ...
    else if(mUnitManager->getPlayer()->isLeaning())
    {
        tPrimary = 5;
    }
    else if(mUnitManager->getPlayer()->isRolledOver())
    {
        tPrimary = 6;
    }
    else if(mUnitManager->getPlayer()->isHeadswinging())
    {
        tPrimary = 7;
    }
    else if(mUnitManager->getPlayer()->isHeadbanging())
    {
        tPrimary = 8;
    }
    else if(mUnitManager->getPlayer()->isButtswinging())
    {
        tPrimary = 9;
    }
    else if(mUnitManager->getPlayer()->isWingwaving())
    {
        tPrimary = 10;
    }
    else if(mUnitManager->getPlayer()->isMoonwalking())
    {
        tPrimary = 11;
    }
    else if(mUnitManager->getPlayer()->isThrillering())
    {
        tPrimary = 12;
    }
    else if(mUnitManager->getPlayer()->isPoint())
    {
        tPrimary = 13;
    }
    else if(mUnitManager->getPlayer()->isStretch())
    {
        tPrimary = 14;
    }
    else if(mUnitManager->getPlayer()->isRoaring())
    {
        tPrimary = 15;
    }
    else if(mUnitManager->getPlayer()->isRofling())
    {
        tPrimary = 16;
    }
    else if(mUnitManager->getPlayer()->isCurled())
    {
        tPrimary = 17;
    }
    else if(mUnitManager->getPlayer()->isFainting())
    {
        tPrimary = 18;
    }
    ...
    if(mUnitManager->getPlayer()->isNodHead())
    {
        tSecondary = 1;
    }
    else if(mUnitManager->getPlayer()->isShakeHead())
    {
        tSecondary = 2;
    }
    else if(mUnitManager->getPlayer()->isNodHeadSlow())
    {
        tSecondary = 3;
    }
    else if(mUnitManager->getPlayer()->isShakeHeadSlow())
    {
        tSecondary = 4;
    }
    else if(mUnitManager->getPlayer()->isHeadTilt())
    {
        tSecondary = 5;
    }
    else if(mUnitManager->getPlayer()->isLick())
    {
        tSecondary = 6;
    }
    else if(mUnitManager->getPlayer()->isNuzzle())
    {
        tSecondary = 7;
    }
    else if(mUnitManager->getPlayer()->isSniff())
    {
        tSecondary = 8;
    }
    else if(mUnitManager->getPlayer()->isTailFlick())
    {
        tSecondary = 9;
    }
    else if(mUnitManager->getPlayer()->isLaugh())
    {
        tSecondary = 10;
    }
    else if(mUnitManager->getPlayer()->isChuckle())
    {
        tSecondary = 11;
    }
    ...
    if(target == TOKEN_NULL)
    {
        if(tFlags == lastFlags && tPrimary == lastPrimaryFlags &&
            tSecondary == lastSecondaryFlags)
        {
            return;
        }
...
void MagixNetworkManager::processEmote(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
...
void MagixNetworkManager::sendEmote(const String &emote)
{
    RakNet::BitStream tBitStream;
    ...
    mPeer->Send(&tBitStream, MEDIUM_PRIORITY, UNRELIABLE, 0,
        UNASSIGNED_SYSTEM_ADDRESS, true);
}


void MagixNetworkManager::processAndSendNewPlayerUpdate(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
...
void MagixNetworkManager::sendNewPlayerProcedure(const OwnerToken &target)
{
    ...
    {
        if(mUnitManager->getPlayer()->getEquip(i) != "")
        {
            sendItemEquip(mUnitManager->getPlayer()->getEquip(i), i, target, true);
        }
    }
...
void MagixNetworkManager::sendNewPlayerAloneProcedure()
{
    for(int i = 0; i < MAX_EQUIP; i++)
    {
        if(mUnitManager->getPlayer()->getEquip(i) != "")
        {
            sendItemEquip(mUnitManager->getPlayer()->getEquip(i), i, true);
        }
    }
}


void MagixNetworkManager::processPlayerDisconnected(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    mChatManager->message(tName + (tIsProper ? " has disconnected." : " timed out."),
        (mUnitManager->isPartyMember(tUnit) ? 0 : (mChatManager->getChannel() == 0 ?
    CHAT_LOCAL : CHAT_GENERAL)));
...
void MagixNetworkManager::processWorldInfo(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    if(mUnitManager->getPlayer()->hasPet())
    {
        ...
        const Vector3 tPosition = mUnitManager->getPlayer()->getPosition() +
            Vector3(Math::RangeRandom(-20, 20), tC.flying ? (Real)15 : (Real)0, -25);
...
void MagixNetworkManager::processAndSendMapInfo(Packet *p)
{
...
void MagixNetworkManager::processAttack(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
...
void MagixNetworkManager::sendAttack(const String &attack)
{
    RakNet::BitStream tBitStream;
    ...
    mPeer->Send(&tBitStream, HIGH_PRIORITY, UNRELIABLE, 0, UNASSIGNED_SYSTEM_ADDRESS,
        true);
}


void MagixNetworkManager::processMapChange(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    const String tName = (tUnit->getUser() == "" ? tUnit->getName() : tUnit->getUser());
    mChatManager->message(tName + " has left.",
        (mUnitManager->isPartyMember(tUnit) ? 0 : (mChatManager->getChannel() == 0 ?
    CHAT_LOCAL : CHAT_GENERAL)));
    mUnitManager->deleteUnit(tUnit);
}
...
void MagixNetworkManager::sendMapChange(const String &mapName,
...
void MagixNetworkManager::sendValidateName()
{
    //EASTER EGGS LOL
    for(int i = 0; i < MAX_EQUIP; i++)
    {
        ...
        else if(mUnitManager->getPlayer()->getEquip(i) == "Egg4")
        {
            ...
            if(tNextSlot > 0)
            {
                mUnitManager->equipItem(mUnitManager->getPlayer(), "clawsR", tNextSlot);
            }
...
void MagixNetworkManager::processPlayerData(Packet *p)
{
    playerListUpdated = true;
    ...
    if(tArrive)
    {
        ...
        for(vector<PlayerData>::type::iterator it = playerList.begin(); it != playerList.end();
            it++)
        {
            PlayerData *tData = &*it;
    ...
    else
    {
        ...
        if(mUnitManager->removePartyMember(tName))
        {
            mChatManager->message(
                "Party Member " + String(tName) + " has disconnected.", CHAT_PARTY);
        }
    }
}


bool MagixNetworkManager::popPlayerListUpdated()
{
...
void MagixNetworkManager::processBio(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    if(tIsRequest)
    {
        ...
        mPeer->Send(&tSendStream, LOW_PRIORITY, UNRELIABLE, 0,
            UNASSIGNED_SYSTEM_ADDRESS, true);
    }
    else
    {
        ...
        if(targetBioUpdated == "")
        {
            targetBioUpdated = "None";
        }
    }
}


void MagixNetworkManager::sendBioRequest(const String &name)
{
    RakNet::BitStream tBitStream;
    ...
    mPeer->Send(&tBitStream, LOW_PRIORITY, UNRELIABLE, 0, UNASSIGNED_SYSTEM_ADDRESS,
        true);
}


const String MagixNetworkManager::popTargetBioUpdated()
{
...
void MagixNetworkManager::sendCreateAccount(const String &arg1, const String &arg2,
...
void MagixNetworkManager::processCreateAccount(Packet *p)
{
    ...
    networkMessage = NetworkMessage(
        tSuccess ? NETWORK_CREATEACCOUNTSUCCESS : NETWORK_CREATEACCOUNTFAIL);
}


void MagixNetworkManager::sendEditAccount(const String &arg1, const String &arg2,
    const String &arg3)
{
    RakNet::BitStream tBitStream;
    ...
    mPeer->Send(&tBitStream, HIGH_PRIORITY, RELIABLE_SEQUENCED, 0,
        UNASSIGNED_SYSTEM_ADDRESS, true);
}


void MagixNetworkManager::processEditAccount(Packet *p)
{
    if(!isMainServerConnection)
    {
        return;
    ...
    tBitStream.Read(tSuccess);
    networkMessage = NetworkMessage(
        tSuccess ? NETWORK_EDITACCOUNTSUCCESS : NETWORK_EDITACCOUNTFAIL);
}


void MagixNetworkManager::sendLogon(const String &arg1, const String &arg2)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processLogon(Packet *p)
{
    if(!isMainServerConnection)
    {
        return;
    ...
    networkMessage = NetworkMessage(
        tSuccess ? NETWORK_LOGONSUCCESS : NETWORK_LOGONFAIL);
}


const NetworkMessage MagixNetworkManager::popNetworkMessage()
{
...
void MagixNetworkManager::processLoadChar(Packet *p)
{
    if(!isMainServerConnection)
    {
        return;
    ...
    if(tIndex != mCharScreenManager->getCharID())
    {
        return;
    }
    ...
    if(tSuccess)
    {
        ...
        if(tHasHP)
        {
            ...
            if(tPart.size() > 1)
            {
                mUnitManager->getPlayer()->setHP(tMaxHP == 0 ? 500 : tMaxHP, 0);
            }

            if(tPart.size() > 0)
            {
                mUnitManager->getPlayer()->addHP(StringConverter::parseInt(tPart[0]));
            }
        }
        else
        {
            mUnitManager->getPlayer()->setHP(500);
        }
    ...
    {
        mCharScreenManager->updateChar("");
    }

    mCharScreenManager->refreshPage();
    networkMessage = NetworkMessage(
        (tSuccess ? NETWORK_LOADCHARSUCCESS : NETWORK_LOADCHARFAIL));
}


void MagixNetworkManager::sendLoadChar(const String &username, const short &charID)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processCreateChar(Packet *p)
{
    if(!isMainServerConnection)
    {
        return;
    ...
    tBitStream.Read(tSuccess);
    networkMessage = NetworkMessage(
        (tSuccess ? NETWORK_CREATECHARSUCCESS : NETWORK_CREATECHARFAIL));
}


void MagixNetworkManager::sendCreateChar(const String &username, const String &name,
    const String &data)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendDeleteChar(const String &username, const short &charID)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processEditChar(Packet *p)
{
    if(!isMainServerConnection)
    {
        return;
    ...
    tBitStream.Read(tSuccess);
    networkMessage = NetworkMessage(
        (tSuccess ? NETWORK_EDITCHARSUCCESS : NETWORK_EDITCHARFAIL));
}


void MagixNetworkManager::sendEditChar(const String &username, const short &charID,
    const String &data)
{
    RakNet::BitStream tBitStream;
...
bool MagixNetworkManager::processKick(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    char tName[16];
    ...
    if(tPlayerName == tCompareName)
    {
        ...
        if(tIsBanned)
        {
            mDef->saveBanFile(true, tNumDays);
        }
...
void MagixNetworkManager::sendKick(const String &name, bool isBan,
    const unsigned short numDays)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processItemDrop(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
...
void MagixNetworkManager::sendItemDrop(const String &meshName, const Real &x, const Real &z)
{
    RakNet::BitStream tBitStream;

    tBitStream.Write(MessageID(ID_ITEMDROP));
    tBitStream.Write(OwnerToken(TOKEN_NULL));
    tBitStream.Write(mItemManager->getNextEmptyID(mUnitManager->getPlayer()->getIndex() *
        100));
    stringCompressor->EncodeString(meshName.c_str(), 16, &tBitStream);
    tBitStream.Write(x);
    tBitStream.Write(z);

    mPeer->Send(&tBitStream, MEDIUM_PRIORITY, RELIABLE, 2, UNASSIGNED_SYSTEM_ADDRESS,
        true);
}


void MagixNetworkManager::sendAllItems(const OwnerToken &token)
{
    ...
    for(int i = 0; i < (int)tList.size(); i++)
    {
        ...
        mPeer->Send(&tBitStream, LOW_PRIORITY, RELIABLE, 2, UNASSIGNED_SYSTEM_ADDRESS,
            true);
    }
}


void MagixNetworkManager::processItemPickup(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    unsigned short tItemID;
...
void MagixNetworkManager::sendItemPickup(const unsigned short &iID)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processItemEquip(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
...
void MagixNetworkManager::sendItemEquip(const String &meshName,
    const unsigned short &slotID, const OwnerToken &target, bool dontTellMainServer)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processItemUnequip(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
...
void MagixNetworkManager::sendItemUnequip(const unsigned short &slotID)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendImReady()
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processGodSpeak(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    char tCaption[512];
...
void MagixNetworkManager::sendGodSpeak(const String &caption)
{
    const String tCaption = "<<(" USER_ADMIN_TEXT ")" +
        mUnitManager->getPlayer()->getUser() + ">>" + caption;
    mChatManager->message(tCaption);
...
bool MagixNetworkManager::processForceLogout(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    char tName[16];
...
void MagixNetworkManager::processEvent(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    unsigned char type;
    ...
    if(type == EVENT_EARTHQUAKE)
    {
        mSkyManager->setWeatherEffect("Earthquake");
    }
...
void MagixNetworkManager::sendEvent(const unsigned char &type)
{
    RakNet::BitStream tBitStream;
    ...
    mPeer->Send(&tBitStream, MEDIUM_PRIORITY, RELIABLE, 1, UNASSIGNED_SYSTEM_ADDRESS,
        true);
}


void MagixNetworkManager::sendMassBlock(const OwnerToken &target)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processInfo(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    unsigned char type;
    ...
    if(type == INFO_PARTYREQUEST)
    {
        ...
        networkMessage = NetworkMessage(NETWORK_PARTYREQUEST);
        return;
    }
    ...
    else if(type == INFO_PARTYACCEPTED)
    {
        ...
        for(int i = 0; i < MAX_PARTYMEMBERS; i++)
        {
            ...
            if(tMemberToken == mUnitManager->getPartyInviter())
            {
    ...
    else if(type == INFO_PARTYJOINED)
    {
        ...
        if(tToken == mUnitManager->getPartyInviter())
        {
            ...
            if(tPartyOverload)
            {
                sendInfo(INFO_PARTYKICK, tToken);
            }
        }
...
void MagixNetworkManager::sendInfo(const unsigned char &type, const OwnerToken &target)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendPartyAccepted(const OwnerToken &target)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendPartyJoined(const OwnerToken &target, bool requestPartyList)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendPartyLeave()
{
    ...
    for(int i = 0; i < (int)tParty.size(); i++)
    {
        sendInfo(INFO_PARTYLEAVE, tParty[i].first);
    }
...
void MagixNetworkManager::sendAcknowledge(const unsigned char &type,
...
void MagixNetworkManager::sendSkillUpdate(const String &type, const unsigned char &stock)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendHPUpdate(bool sendMaxHP)
{
    RakNet::BitStream tBitStream;

    tBitStream.Write(MessageID(ID_UPDATEHP));
    tBitStream.Write((int)
        mUnitManager->getPlayer()->getIsWounded() ? 0 : mUnitManager->getPlayer()->getHP());
    tBitStream.Write(sendMaxHP);

    if(sendMaxHP)
    {
        tBitStream.Write((int)mUnitManager->getPlayer()->getMaxHP());
    }
...
void MagixNetworkManager::sendPetUpdate()
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterCreate(const String &type, const Vector3 &position,
    bool isPet)
{
    const unsigned short tID = mCritterManager->getNextEmptyID((
        mUnitManager->getPlayer()->getIndex() - 1) * 200);
...
void MagixNetworkManager::sendCritterCreate(const unsigned char &typeID,
...
void MagixNetworkManager::disownCritters(const OwnerToken &target)
{
    ...
    for(int i = 0; i < (int)tList.size(); i++)
    {
        ...
        if(tC->getOwnerIndex() == target && !tC->getIsDead())
        {
            if(tC->getIsPet())
            {
                mUnitManager->deleteCritter(tC->getID());
            }
            else
            {
...
void MagixNetworkManager::sendCritterTakeover(const unsigned short &iID,
    const OwnerToken &token, bool fastestFirst)
{
    RakNet::BitStream tBitStream;
    ...
    tBitStream.Write(true);
    tBitStream.Write(
        (token == TOKEN_NULL ? (OwnerToken)mUnitManager->getPlayer()->getIndex() : token));
    ...
    mPeer->Send(&tBitStream, MEDIUM_PRIORITY, RELIABLE, 4, UNASSIGNED_SYSTEM_ADDRESS,
        true);
}


void MagixNetworkManager::processCritterOwnage(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    ...
    if(tIsCreate)
    {
        ...
        if(tIsWorldCritter)
        {
            tBitStream.Read(tTypeID);
        }
        else
        {
            stringCompressor->DecodeString(tType, 32, &tBitStream);
        }
        ...
        if(tToken == mUnitManager->getPlayer()->getIndex())
        {
            mCritterManager->popSentID(tCritterID);
        }
        ...
        if(tIsWorldCritter)
        {
            tC = mUnitManager->createCritter(tCritterID, tTypeID, tPosition,
                (tToken == mUnitManager->getPlayer()->getIndex() ? 0 : tToken));
        }
        else
        {
            tC = mUnitManager->createCritter(tCritterID, tType, tPosition,
                (tToken == mUnitManager->getPlayer()->getIndex() ? 0 : tToken));
        }

        if(tIsPet)
        {
            mCritterManager->setIsPet(tC);
        }
        ...
        if(tC && tIsPet && tC->imTheOwner())
        {
            ...
            if(tFlags->shrink)
            {
                sendCritterShrink(tC->getID(), true);
            }

            if(tFlags->evolve)
            {
                sendCritterShrink(tC->getID(), true, true);
            }
        }
    ...
    if(tIsTakeover)
    {
        ...
        if(tFastestFirst && tC->getOwnerIndex() >= 0)
        {
            return;
        }
...
void MagixNetworkManager::sendCritterHit(const unsigned short &iID, const Real &damage,
    const Vector3 &force)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterDecision(const unsigned short &iID, const Vector3 &target)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterAttackDecision(const unsigned short &iID,
    const unsigned short &targetID, const unsigned char &attackID, bool targetIsCritter)
{
    RakNet::BitStream tBitStream;
    ...
    if(targetIsCritter)
    {
        tBitStream.Write(targetID);
    }
    else
    {
        tBitStream.Write((OwnerToken)targetID);
    }
...
void MagixNetworkManager::sendCritterKilled(const unsigned short &iID, const short &killer)
{
    RakNet::BitStream tBitStream;
    ...
    tBitStream.Write(false);
    tBitStream.Write(true);
    const OwnerToken tToken = killer == -1 ? TOKEN_NULL :
        (killer == 0 ? mUnitManager->getPlayer()->getIndex() : killer);
    tBitStream.Write(tToken);
...
void MagixNetworkManager::sendCritterTamed(const unsigned short &iID)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterShrink(const unsigned short &iID, bool flag, bool isEvolve)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processCritterInfo(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    unsigned short tCritterID;
    ...
    if(tIsDecision)
    {
        ...
        if(tIsAttack)
        {
            ...
            else
            {
                ...
                if(tTargetID == mUnitManager->getPlayer()->getIndex())
                {
                    tTarget = mUnitManager->getPlayer();
                }
                else
                {
                    tTarget = mUnitManager->getUnitByIndex(tTargetID);
                }
            }
    ...
    if(tIsKilled)
    {
        ...
        mUnitManager->killAndRewardCritter(tCritter,
            (tToken == mUnitManager->getPlayer()->getIndex()));
        return;
    }
    ...
    if(tIsTamed)
    {
        ...
        if(tImTheOwner)
        {
            ...
            {
                mChatManager->message("You have tamed more than one pet, only the newest one will follow you.");
            }
    ...
    if(tIsShrunk)
    {
        ...
        if(tIsTrue)
        {
            if(tIsEvolve)
            {
                tCritter->getObjectNode()->setScale(15, 15, 15);
            }
            else
            {
                const Real tScale = (Real)(tCritter->getLength() > 50 ? 0.1 :
                    (1 - tCritter->getLength()*0.018));
        ...
        else
        {
            ...
            const Real tScale = (Real)((tCritter->getIsPet() && tC.scale > 1.5) ? 1.5 : tC.scale);
            tCritter->getObjectNode()->setScale(tScale, tScale, tScale);
        }
...
void MagixNetworkManager::sendMyCritters(const OwnerToken &target)
{
    ...
    for(int i = 0; i < (int)tList.size(); i++)
    {
        ...
        if(!tC->getIsDead())
        {
            if(tC->getIsWorldCritter())
            {
                sendCritterUpdate(target, tC->getID(), tC->getWorldCritterID(), tC->getPosition(),
                    tC->getHPRatio());
            }
            else
            {
                sendCritterUpdate(target, tC->getID(), tC->getCritterType(), tC->getPosition(),
                    tC->getHPRatio(), tC->getIsPet());
    ...
    if(tPet)
    {
        ...
        if(tFlags->shrink)
        {
            sendCritterShrinkTargeted(target, tPet->getID());
        }

        if(tFlags->evolve)
        {
            sendCritterShrinkTargeted(target, tPet->getID(), true);
        }
    }
}


void MagixNetworkManager::sendCritterUpdate(const OwnerToken &target,
    const unsigned short &iID, const unsigned char &typeID, const Vector3 &position,
    const Real &hpRatio, const OwnerToken &token)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterUpdate(const OwnerToken &target,
    const unsigned short &iID, const String &type, const Vector3 &position, const Real &hpRatio,
    bool isPet, const OwnerToken &token)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterUpdateRequest(const OwnerToken &target,
    const unsigned short &iID)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterShrinkTargeted(const OwnerToken &target,
    const unsigned short &iID, bool isEvolve)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processCritterTargetedInfo(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    unsigned short tCritterID;
    ...
    if(tIsUpdate)
    {
        ...
        if(tIsWorldCritter)
        {
            tCritter = mUnitManager->createCritter(tCritterID, tTypeID, tPosition, tToken);
        }
        else
        {
    ...
    if(tIsUpdateRequest)
    {
        ...
        {
            sendCritterUpdate(tToken, tCritterID, tCritter->getWorldCritterID(),
                tCritter->getPosition(), tCritter->getHPRatio(), tOwner > 0 ? tOwner :
                OwnerToken(TOKEN_NULL));
        }
        else
        {
            sendCritterUpdate(tToken, tCritterID, tCritter->getCritterType(), tCritter->getPosition(),
                tCritter->getHPRatio(), tCritter->getIsPet(), tOwner > 0 ? tOwner :
                OwnerToken(TOKEN_NULL));
        }
    ...
    if(tIsShrunk)
    {
        ...
        {
            tCritter->getObjectNode()->setScale(0.1, 0.1, 0.1);
        }
...
void MagixNetworkManager::sendPlayerUpdate(const OwnerToken &target)
{
    RakNet::BitStream tBitStream;
    ...
    if(!tWounded)
    {
        tBitStream.Write((unsigned short)mUnitManager->getPlayer()->getHP());
    }
...
void MagixNetworkManager::sendPlayerHit(const OwnerToken &unitID, const Real &damage,
    const Vector3 &force)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendPlayerWounded()
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendPlayerUnwounded()
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processPlayerInfo(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    OwnerToken tToken;
    ...
    if(tIsWounded)
    {
        ...
        if(mUnitManager->isPartyMember(unit))mChatManager->message(
            unit->getName() + " is wounded!", CHAT_PARTY);
    }
    ...
    if(tIsUnwounded)
    {
        ...
        {
            mChatManager->message(
                unit->getName() + " recovered from the wound.", CHAT_PARTY);
        }
    }
}


void MagixNetworkManager::sendItemStash(const unsigned short &slot)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendItemUnstash(const unsigned short &line, const unsigned short &slot)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processItemStash(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    bool tIsStash = false;
...
void MagixNetworkManager::processFindPlayer(Packet *p)
{
    RakNet::BitStream tBitStream(p->data, p->length, false);
    MessageID tID;
    char tName[16];
    ...
    {
        mChatManager->message(String(tName) + " is not online.");
    }
}


void MagixNetworkManager::sendFindPlayer(const String &name)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendFindToken(const OwnerToken &token)
{
    RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processServerList(Packet *p)
{
    RakNet::BitStream tReceiveBit(p->data, p->length, false);
    MessageID tMessage;
...
void MagixNetworkManager::processServerConnected(Packet *p)
{
    RakNet::BitStream tReceiveBit(p->data, p->length, false);
    MessageID tMessage;
    bool tIsConnect;
...
void MagixNetworkManager::processMaintenance(Packet *p)
{
    RakNet::BitStream tReceiveBit(p->data, p->length, false);
    MessageID tMessage;
    ...
    mChatManager->message(
        "Servers are restarting in " + StringConverter::toString((int)tCount) +
        " seconds. Please log out to avoid data loss. Thank you C:");
}


void MagixNetworkManager::sendViolation(const String &info)
{
    RakNet::BitStream tBitStream;
