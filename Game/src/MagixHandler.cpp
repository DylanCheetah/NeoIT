/*
Notes:
Appears to be the "manager of all managers"! XD
*/
...
void MagixHandler::initialize(SceneManager *sceneMgr, RenderWindow *window)
{
    ...
    mWorld->initialize(sceneMgr, window, mDef, mGameStateManager, mSoundManager,
        mCollisionManager, mSkyManager);
    mCritterManager->initialize(sceneMgr, mDef, mGameStateManager, mEffectsManager,
        mCollisionManager, mSoundManager, mWorld, mCamera);
    mUnitManager->initialize(sceneMgr, mDef, mGameStateManager, mWorld,
        mEffectsManager, mCollisionManager, mSoundManager, mItemManager,
        mCritterManager, mCamera);
    mNetworkManager->initialize(mDef, mGameStateManager, mItemManager,
        mCritterManager, mUnitManager, mWorld, mSkyManager, mGui->getChatManager(),
        mGui->getAlertBox(), mGui->getCharScreenManager());
    mOwnershipManager->initialize(mDef, mGameStateManager, mCritterManager,
        mUnitManager, mNetworkManager, mWorld, mGui->getChatManager(), mSkyManager);
    mGui->initialize(sceneMgr, window, mDef, mGameStateManager, mItemManager,
        mCritterManager, mUnitManager, mNetworkManager, mCamera, mSoundManager,
        mEffectsManager, mCollisionManager);
    mCampaignManager->initialize(mDef, mGameStateManager, mGui, mUnitManager,
        mWorld, mSkyManager, mCamera, mSoundManager, mCollisionManager,
        mEffectsManager);
...
void MagixHandler::shutdown()
{
    ...
    if(!mGameStateManager->isCampaign() &&
        mGameStateManager->getCampaignName() == "" &&
        mWorld->getWorldName() != "" && mGameStateManager->isInGame())
    {
        mDef->saveSavePoint(mWorld->getWorldName(),
            Vector2(getPlayer()->getPosition().x, getPlayer()->getPosition().z),
...
void MagixHandler::resetScene(bool startScene, const String &mapChange)
{
    if(!startScene)
    {
        mSoundManager->reset();
...
bool MagixHandler::update(const FrameEvent &evt)
{
    ...
    case GAMESTATE_STARTCHARSCREEN:
        ...
        {
            mCamera->getActiveCamNode()->detachObject(mCamera->getCamera());
        }

        mCamera->getCamera()->setPosition(3200, 120, 2770);
        mCamera->getCamera()->setOrientation(Quaternion(Degree(-40),
            Vector3::UNIT_Y));
        mSkyManager->showSky(true);
        ...
        CompositorManager::getSingleton().setCompositorEnabled(
            mWindow->getViewport(0), "Bloom", mGui->isBloom());
    ...
    case GAMESTATE_RETURNSTARTSCREEN:
        if(mGameStateManager->getGameStarted() &&
            mGameStateManager->getCampaignName() == "" &&
            mWorld->getWorldName() != "" && getPlayer()->getObjectNode())
            mDef->saveSavePoint(mWorld->getWorldName(),
            Vector2(getPlayer()->getPosition().x,
            getPlayer()->getPosition().z), mGameStateManager->getDimension());
        mWorld->unloadWorld();
    ...
    case GAMESTATE_STARTCONNECTION:
        ...
        if(!getPlayer()->getWingEnt())
        {
            getPlayer()->pushDefaultAttackList("Somersault");
        }
        else
        {
            getPlayer()->pushDefaultAttackList("Wingslash");
        }
        ...
        if(mNetworkManager->connect())
        {
            mGameStateManager->setGameState(GAMESTATE_CONNECTING);
        }
    ...
    case GAMESTATE_CONNECTING:
        ...
        if(mGameStateManager->getGameState() == GAMESTATE_STARTGAME)
        {
            updateEverything(evt);
        }
        break;
...
void MagixHandler::updateEverything(const FrameEvent &evt)
{
    updateCommand();
...
void MagixHandler::updateCommand()
{
    ...
    {
        mDef->saveHomePoint(mWorld->getWorldName(),
            Vector2(getPlayer()->getPosition().x, getPlayer()->getPosition().z), mGameStateManager->getDimension());
        mGui->getChatManager()->message(
            "Home Point has been set to the current position.");
    }
    else if(tCommand == COMMAND_GOHOME)
    {
        ...
        if(tMap == mWorld->getWorldName() &&
            tDimension == mGameStateManager->getDimension())
        {
            getPlayer()->setPosition(tPoint.x, 200, tPoint.y);
        ...
        {
            mUnitManager->setMapChange(true, tMap, true,
                Vector3(tPoint.x, 0, tPoint.y));
            mGameStateManager->setDimension(tDimension);
        }
    }
    ...
    {
        Real pitch = 0;
        Real yaw = getPlayer()->getYaw().valueDegrees();
        Real roll = 0;
        Vector3 rot(pitch, yaw, roll);
        mGui->getChatManager()->message(StringConverter::toString(getPlayer()->getPosition()) +
            ";1 1 1;" + StringConverter::toString(rot));
    }
    else if(tCommand == COMMAND_EARTHQUAKE)
    {
        if(mDef->isAdmin)
        {
            mNetworkManager->sendEvent(EVENT_EARTHQUAKE);
        }
    }
    else if(tCommand == COMMAND_GOTO)
    {
        ...
        {
            mUnitManager->setMapChange(true, tMap, (tPart.size() > 1),
                (tPart.size() > 1 ? StringConverter::parseVector3(tPart[1]) :
                Vector3::ZERO));
        }
    }
}
...
void MagixHandler::updateMapChange(const FrameEvent &evt)
{
    if(mGameStateManager->popMapChangeDone())
    {
        ...
        mWorld->update();
        mSoundManager->update(evt);
        mGui->getChatManager()->message("---" + mDef->getMapName(
            mWorld->getWorldName()) + "---");
    }
    else
    {
        ...
        {
            getPlayer()->setPosition(tMapChange.point.x,
                (tMapChange.hasVectY ? tMapChange.point.y : 200), tMapChange.point.z);
            mUnitManager->clampUnitToTerrain(getPlayer(), !tMapChange.hasVectY);
        }

        if(!mGameStateManager->isCampaign())
        {
            mNetworkManager->sendMapChange(tMapChange.mapName,
                mGameStateManager->getDimension());
            mDef->saveSavePoint(mWorld->getWorldName(),
                Vector2(getPlayer()->getPosition().x,
                getPlayer()->getPosition().z), mGameStateManager->getDimension());
...
void MagixHandler::updateCredits(const FrameEvent &evt)
{
    mGui->update(evt);
    mSoundManager->update(evt);
...
void MagixHandler::updateGameOver(const FrameEvent &evt)
{
    mGui->update(evt);
    mSoundManager->update(evt);
}
...
void MagixHandler::updateLogo(const FrameEvent &evt)
{
    mGui->update(evt);
}


void MagixHandler::updateStartScreen(const FrameEvent &evt)
{
    mNetworkManager->update(evt);
    mGui->update(evt);
    //mSoundManager->update(evt);
...
void MagixHandler::updateCharScreen(const FrameEvent &evt)
{
    mNetworkManager->update(evt);
    mGui->update(evt);
...
MagixPlayer* MagixHandler::getPlayer()
{
    return mUnitManager->getPlayer();
}


String MagixHandler::getDebugText()
{
    return mDebugText;
}


MagixGUI* MagixHandler::getMagixGUI()
{
    return mGui;
}


MagixInputManager* MagixHandler::getInputManager()
{
    return mGui->getInputManager();
}


MagixUnitManager* MagixHandler::getUnitManager()
{
    return mUnitManager;
}


MagixCamera* MagixHandler::getCamera()
{
    return mCamera;
}


MagixExternalDefinitions* MagixHandler::getExternalDefinitions()
{
    return mDef;
}


MagixNetworkManager* MagixHandler::getNetworkManager()
{
    return mNetworkManager;
}


unsigned short MagixHandler::getCameraMode()
{
    return mCamera->getCameraMode();
}


unsigned short MagixHandler::getInputMode()
{
    return getInputManager()->getInputMode();
}


MagixGameStateManager* MagixHandler::getGameState()
{
    return mGameStateManager;
}


MagixEffectsManager* MagixHandler::getEffectsManager()
{
