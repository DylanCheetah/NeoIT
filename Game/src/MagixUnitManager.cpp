...
MagixUnitManager::~MagixUnitManager()
{
    ...
    if(mChatBubbleAttr)
    {
        delete mChatBubbleAttr;
    }
}


void MagixUnitManager::initialize(SceneManager *sceneMgr, MagixExternalDefinitions *def,
    MagixGameStateManager *gameStateMgr, MagixWorld *world, MagixEffectsManager *effectsMgr,
    MagixCollisionManager *collisionMgr, MagixSoundManager *soundMgr,
...
void MagixUnitManager::reset(bool destroyPlayer, bool resetPosition)
{
    ...
    {
        if(mPlayer->getAutoTrackObject())
        {
            mPlayer->lockOn(0);
        }

        if(mPlayer->getAutoAttackTarget())
        {
        ...
        if(mPlayer->getPetFlags()->attackTarget)
        {
...
void MagixUnitManager::resetPlayerPosition()
{
    ...
    mPlayer->setPosition(tVect.x + Math::RangeRandom(0, tVect.z), 700,
        tVect.y + Math::RangeRandom(0, tVect.z));
...
{
    const Quaternion camOrientation = (mCamera ? mCamera->getCameraOrientation() :
        Quaternion::ZERO);
    ...
    if((isPlayer || !unit->allocFlags.hasOwnGroundHeight) &&
        unit->isPositionChanged(!unit->isAntiGravity()))
    {
    ...
    if(!tIsInRange)
    {
        ...
        {
            unit->getObjectNode()->setVisible(false);
        }
    ...
    if(tIsInRange)
    {
        if(isPlayer)
        {
            updateCollBoxCollisions(unit, evt);
        }
    ...
    if(unit->popHasStoppedAttack())
    {
        ...
        if(unit->getWingEnt())
        {
            mEffectsManager->destroyRibbonTrailByEntity(unit->getWingEnt());
        }
        ...
        if(tAttack.name != "")
        {
            if(tIsInRange)
            {
                for(int i = 0; i < (int)tAttack.FX.size(); i++)
                {
                    ...
                    if(tEnt)
                    {
                        mEffectsManager->createRibbonTrail(tEnt, tFX.bone, tFX.trailMat,
                            tFX.trailWidth, tFX.colour, tFX.colourChange, tFX.trailHeadMat);
                    }
                }
            }
            ...
            if(tAttack.singleTarget)
            {
                if(isPlayer || mGameStateManager->isCampaign())
                {
                    ...
                    if(target)
                    {
                        ...
                        {
                            bool tIsAlly = static_cast<MagixLiving*>(target)->matchAlliance(
                                unit->getAlliance());
                    ...
                    if(target)
                    {
                        const Vector3 tForce = tAttack.hitForce == Vector3::ZERO ? Vector3::ZERO :
                            tAttack.hitForce.getRotationTo(target->getPosition() - unit->getPosition())*
                            tAttack.hitForce;
                        ...
                        if(target->getType() == OBJECT_CRITTER)
                        {
                            ...
                            if(isPlayer&&!mGameStateManager->isCampaign())
                            {
                                mCritterManager->pushHitQueue(HitInfo(tC->getID(), tHP, tForce,
                                    tC->imTheOwner()));
                            }
                        ...
                        else if(target->getType() == OBJECT_UNIT || target == mPlayer)
                        {
                            ...
                            if(isPlayer&&!mGameStateManager->isCampaign())
                            {
                                ...
                                else if(tUnit->isIndexedUnit())
                                {
                                    pushHitQueue(HitInfo(
                                        static_cast<MagixIndexedUnit*>(tUnit)->getIndex(), tHP, tForce));
                                }
                            }
                        }
                            ...
                            if(tAttack.name == "Tame")
                            {
                                ...
                                if(target->getType() == OBJECT_CRITTER)
                                {
                                    ...
                                    if(tSuccess)
                                    {
                                        ...
                                        mEffectsManager->createParticle(tC->getObjectNode(),
                                            "TameSuccess", 1);
                                    }
                                    else
                                    {
            ...
            else
            {
                if(isPlayer || mGameStateManager->isCampaign())
                {
                    mCollisionManager->createCollision(unit->getObjectNode(), tAttack.range,
                        tAttack.hitForce, tAttack.offset, (short)Math::RangeRandom(tAttack.hpMin,
                        tAttack.hpMax), unit->getAlliance(), tAttack.hitAlly);
                }
            }

            if(tAttack.moveForce != Vector3::ZERO)
            {
                unit->addForce((tAttack.autoTarget ? doAutoTarget(unit, isPlayer) :
                    unit->getObjectNode()->getOrientation())*tAttack.moveForce);
            }
    ...
    vector<Collision*>::type tHitList = mCollisionManager->getCollisionHitList(unit->getUnitID(),
        unit->getAlliance(), unit->getBodyEnt()->getWorldBoundingBox());
    ...
    for(vector<Collision*>::type::iterator it = tHitList.begin(); it != tHitList.end(); it++)
    {
        ...
        if(isPlayer || coll->mNode == mPlayer->getObjectNode() ||
            mGameStateManager->isCampaign())
        {
            const Vector3 tForce = coll->getForce(false) == Vector3::ZERO ? Vector3::ZERO :
                coll->getForce(true, unit->getPosition());
            ...
            if(!mGameStateManager->isCampaign())
            {
                ...
                {
                    pushHitQueue(HitInfo(static_cast<MagixIndexedUnit*>(unit)->getIndex(), coll->hp,
                        tForce));
                }
            }
            ...
            if(coll->hp < 0)
            {
                if((mPlayer->setPlayerTargetOnHit || !mPlayerTarget) &&
                    coll->mNode == mPlayer->getObjectNode())
                {
...
bool MagixUnitManager::isInRange(MagixUnit *unit)
{
    ...
    if(mCamera)
    {
        ...
        {
            return(unit->getPosition().squaredDistance(mCamera->getCamera()->getPosition()) <
                EARSHOT_SQUARED);
        }
    }
...
void MagixUnitManager::hitUnit(MagixUnit *unit, const Real &damage, const Vector3 &force,
    bool inRange)
{
    if(!unit)
    {
    ...
    for(vector<Wall*>::type::const_iterator it = tHitList.begin(); it != tHitList.end(); it++)
    {
        ...
        if(tWall->isSphere)
        {
            ...
            if(Math::Abs(tX) > 0.1 && Math::Abs(tZ) > 0.1)
            {
                ...
                unit->addPenaltyVelocity(Vector3(Math::Sin(tYaw)*tWall->range.x - tX, 0,
                    Math::Cos(tYaw)*tWall->range.x - tZ));
            }
        }
        else
        {
            ...
            if(tWall->isInside)
            {
                const Real tNearestX = (Math::Abs(tMinX - unit->getPosition().x) <=
                    Math::Abs(tMaxX - unit->getPosition().x) ? tMinX - unit->getPosition().x : tMaxX -
                    unit->getPosition().x);
                const Real tNearestZ = (Math::Abs(tMinZ - unit->getPosition().z) <=
                    Math::Abs(tMaxZ - unit->getPosition().z) ? tMinZ - unit->getPosition().z : tMaxZ -
                    unit->getPosition().z);

                if(Math::Abs(tNearestX) <= Math::Abs(tNearestZ))
                {
                    unit->addPenaltyVelocity(
                        Vector3(tNearestX, 0, 0));
                }
                else
                {
            ...
            {
                if(unit->getPosition().x < tMinX)unit->addPenaltyVelocity(Vector3(tMinX -
                    unit->getPosition().x, 0, 0));
                if(unit->getPosition().x > tMaxX)unit->addPenaltyVelocity(Vector3(tMaxX -
                    unit->getPosition().x, 0, 0));
                if(unit->getPosition().z < tMinZ)unit->addPenaltyVelocity(Vector3(0, 0, tMinZ -
                    unit->getPosition().z));
                if(unit->getPosition().z > tMaxZ)unit->addPenaltyVelocity(Vector3(0, 0, tMaxZ -
                    unit->getPosition().z));
            }
        }
    }
...
{
    Portal *tPortal = mCollisionManager->getPortalHit(
        unit->getBodyEnt()->getWorldBoundingBox());
...
void MagixUnitManager::updateCollBoxCollisions(MagixUnit *unit, const FrameEvent &evt)
{
    ...
    const vector<CollBox*>::type tHitList = mCollisionManager->getCollBoxHitList(
        unit->getPosition(), headHeight);
    ...
    for(vector<CollBox*>::type::const_iterator it = tHitList.begin(); it != tHitList.end(); it++)
    {
        ...
        const Real tYCorrection = (unit->getForce().y < 0 ? unit->getForce().y*
            (1 + evt.timeSinceLastFrame) : 0);
        ...
        else
        {
            ...
            else
            {
                ...
                const Real tNearestX = (Math::Abs(tMinX - unit->getPosition().x) <=
                    Math::Abs(tMaxX - unit->getPosition().x) ? tMinX - unit->getPosition().x :
                    tMaxX - unit->getPosition().x);
                const Real tNearestZ = (Math::Abs(tMinZ - unit->getPosition().z) <=
                    Math::Abs(tMaxZ - unit->getPosition().z) ? tMinZ - unit->getPosition().z :
                    tMaxZ - unit->getPosition().z);
    ...
    const vector<CollSphere*>::type tHitList2 = mCollisionManager->getCollSphereHitList(
        unit->getPosition());
    ...
    for(vector<CollSphere*>::type::const_iterator it = tHitList2.begin(); it != tHitList2.end(); it++)
    {
        ...
        else
        {
            ...
            if(Math::Abs(tX) > 0.1 && Math::Abs(tZ) > 0.1)
            {
                ...
                unit->addPenaltyVelocity(Vector3(Math::Sin(tYaw)*tSphere->range - tX, 0,
                    Math::Cos(tYaw)*tSphere->range - tZ));
            }
...
void MagixUnitManager::updateWaterCollisions(MagixUnit *unit)
{
    ...
    if(tWaterBox)
    {
        ...
        else
        {
            ...
            if(unit->getPosition().y + tNode->getScale().x * 12 > tWaterHeight)
            {
                ...
                {
                    mEffectsManager->createWaterRipple(tNode, tWaterHeight, tNode->getScale().x *
                        10);
                }
            ...
            else if(unit->getPosition().y + tNode->getScale().x * 15 < tWaterHeight)
            {
                ...
                if(!mEffectsManager->hasBreatherOwner(tHeadEnt, "Bubbles"))
                {
                    ...
                    mEffectsManager->createBreather(tHeadEnt, "Bubbles", Vector3(0,
                        (Real)(1.2*tScale), (Real)(-0.4*tScale)), (Real)(tScale*0.2));
                }
            }

            //Splash!
            if(!unit->getIsInWater() && unit->getForce().y < -2 && !unit->getIsSwimming() &&
                !unit->isAntiGravity())
            {
    ...
    {
        mEffectsManager->setBreatherEmissionByOwner(unit->getHeadEnt(),
            Math::RangeRandom(0.1, 4), "Bubbles");
    }
}
...
void MagixUnitManager::updateMapEffects(MagixUnit *unit)
{
    if(mWorld->getMapEffect() != "")
    {
        ...
        if(!unit->allocFlags.mapEffect)
        {
            ...
            mEffectsManager->createBreather(tHeadEnt, mWorld->getMapEffect(),
                Vector3(0, (Real)(1.2*tScale), (Real)(-0.4*tScale)), (Real)(tScale * 1));
...
{
    if((unit->getForce().x != 0 || unit->getForce().z != 0) && !unit->getIsCrouching() &&
        !unit->isAntiGravity() && !unit->getIsInWater() &&
        unit->distanceFromGround() < GROUND_THRESHOLD)
    {
        if(unit->getIsWalking())
        {
            ...
            {
                mSoundManager->playLoopedSound(SOUND_WALK_GRASS, unit->getObjectNode(), 1,
                    0.4);
        ...
        else
        {
            ...
            if(!unit->allocFlags.soundRunGrass)
            {
                mSoundManager->playLoopedSound(SOUND_RUN_GRASS, unit->getObjectNode(), 1,
                    0.4);
    ...
    if(unit->allocFlags.needsSoundRoar && unit->isRoaring())
    {
        mSoundManager->playSound(
            (Math::RangeRandom(0, 1) < 0.5 ? SOUND_ROAR1 : SOUND_ROAR2),
            unit->getObjectNode());
...
void MagixUnitManager::setMapChange(bool changeMap, const String &mapName,
    bool changePoint, const Vector3 &point, bool hasVectY)
{
...
const Quaternion MagixUnitManager::doAutoTarget(MagixUnit *unit, bool targetClosest)
{
    ...
    if(unit->getType() == OBJECT_PLAYER &&
        static_cast<MagixPlayer*>(unit)->getAutoAttackTarget())
    {
        unit->setTarget(static_cast<MagixPlayer*>(unit)->getAutoAttackTarget()->getPosition(),
            TARGET_LOOKAT);
        return (unit->getFaceDirection());
    }
    ...
    if(targetClosest)
    {
        ...
        while(tLast)
        {
            ...
            {
                if(unit->getPosition().squaredDistance(tLast->getPosition()) <=
                    AUTOTARGET_RANGE_SQUARED)
                {
        ...
        for(int i = 0; i < (int)tCritter.size(); i++)
        {
            if(!tCritter[i]->getIsDead())
            {
                ...
                {
                    if(unit->getPosition().squaredDistance(tCritter[i]->getPosition()) <=
                        AUTOTARGET_RANGE_SQUARED)
                    {
...
{
    const bool tTargetAlly = mPlayer->isSkillMode() ? mDef->isSkillTargetsAllies(
        mPlayer->getCurrentSkill()->name) : false;
    ...
    while(tLast)
    {
        if(!mPlayer->getIsLockedOn() || (mPlayer->getIsLockedOn() &&
            mPlayer->getAutoTrackObject() != tLast))
        {
    ...
    for(int i = 0; i < (int)tCritter.size(); i++)
    {
        ...
        {
            if(!mPlayer->getIsLockedOn() || (mPlayer->getIsLockedOn() &&
                mPlayer->getAutoTrackObject() != tCritter[i]))
            {
...
const Real MagixUnitManager::getGroundHeight(const Real &x, const Real &z,
    bool checkWaterCollision, bool checkCollBoxCollision)
{
    ...
    if(checkCollBoxCollision)
    {
        ...
        for(vector<CollSphere*>::type::const_iterator it = tHitList2.begin(); it != tHitList2.end();
            it++)
        {
            CollSphere *tSphere = *it;
...
MagixIndexedUnit* MagixUnitManager::createUnit(const unsigned short iIndex,
    ...
    const String &tuftMesh)
{
    if(!mLastUnit)
    {
        ...
        mLastUnit->createUnit(2, mSceneMgr, bodyMesh, headMesh, maneMesh, tailMesh, wingMesh,
            matName, tuftMesh);
    }
    else
    {
        ...
        mLastUnit->createUnit(tLastID + 1, mSceneMgr, bodyMesh, headMesh, maneMesh, tailMesh,
            wingMesh, matName, tuftMesh);
    }
...
void MagixUnitManager::deleteUnit(MagixIndexedUnit *target)
{
    ...
    if(target->hasMarker)
    {
        OverlayManager::getSingleton().destroyOverlayElement("UnitMarker" +
            StringConverter::toString(target->getUnitID()));
    }
...
void MagixUnitManager::deleteAllUnits()
{
    while(mLastUnit)
    {
        ...
        {
            OverlayManager::getSingleton().destroyOverlayElement("UnitMarker" +
                StringConverter::toString(tLast->getUnitID()));
        }
...
void MagixUnitManager::createChatBubble(MagixUnit *target, const String &caption)
{
    if(target)
    {
        target->createChatBubble(caption, mChatBubbleAttr, 0, (Real)(target == mPlayer ? 1 : 0));
    }
}
...
void MagixUnitManager::destroyUnit(MagixUnit *unit)
{
    if(unit->getObjectNode())
    {
        ...
        while(tLast)
        {
            if(tLast != unit)
            {
                if(tLast->getAutoTrackObject() == unit)
                {
                    tLast->setAutoTrackObject(0);
                }
            }
...
void MagixUnitManager::createPlayerUnit(const String &bodyMesh, const String &headMesh,
    const String &maneMesh, const String &tailMesh, const String &wingMesh,
    const String &matName, const String &tuftMesh)
{
    mPlayer->createUnit(1, mSceneMgr, bodyMesh, headMesh, maneMesh, tailMesh, wingMesh,
        matName, tuftMesh);
    mPlayer->reset();
...
ParticleSystem* MagixUnitManager::createParticleOnUnit(MagixUnit *unit,
    const String &particleName, const Real &duration, bool unresettable)
{
    if(!unit->getObjectNode())
    {
        return 0;
    ...
    return mEffectsManager->createParticle(unit->getObjectNode(), particleName, duration,
        unresettable);
}


ParticleSystem* MagixUnitManager::createParticleOnUnitBone(MagixUnit *unit,
    const String &tBone, const String &particleName, const Real &duration, bool unresettable)
{
    if(!unit->getObjectNode())
    {
        return 0;
    ...
    ParticleSystem *pSys = mEffectsManager->createParticle(unit->getObjectNode(),
        particleName, duration, unresettable, true);
...
bool MagixUnitManager::equipItem(MagixUnit *unit, const String &meshName, const short &slot)
{
    ...
    if(mDef->getItemHasOffset(meshName))
    {
        if(mDef->getItemBone(meshName) == "Head2")
        {
            ...
            {
                offset = Vector3(0, 0.31, 0.19);
            }
        }
        else if(mDef->getItemBone(meshName) == "Eye.L" ||
            mDef->getItemBone(meshName) == "Eye.R")
        {
            char tHeadID = unit->getHeadEnt()->getMesh()->getName().at(4);

            if(tHeadID != '1' && tHeadID != '5')
            {
                offset = Vector3(-0.01, 0.0675, 0.35);
            }
        }
    }
    ...
    if(mDef->getItemParticle(meshName) != "")
    {
        ...
        if(tEnt)
        {
            const std::pair<Vector3, bool> tOffsetAndOnNode = mDef->getItemParticleOffset(
                meshName);

            if(tOffsetAndOnNode.second)
            {
                mEffectsManager->createItemEffect(tEnt, mDef->getItemParticle(meshName), "",
                    tOffsetAndOnNode.first, unit->getObjectNode()->getScale().x);
            }
            else
            {
                mEffectsManager->createItemEffect(tEnt, mDef->getItemParticle(meshName), tBone,
                    tOffsetAndOnNode.first, unit->getObjectNode()->getScale().x);
            }
        }
    }
...
const String MagixUnitManager::unequipItem(MagixUnit *unit, const unsigned short &iID)
{
    ...
    if(tItem != "")
    {
        ...
        if(tItemEffect != "")
        {
            mEffectsManager->destroyItemEffectByOwnerNode(unit->getObjectNode(), tItemEffect);
        }
    }
...
void MagixUnitManager::createItem(const unsigned short &iID, const String &meshName,
...
MagixCritter* MagixUnitManager::createCritter(const unsigned short &iID,
    const unsigned char &worldID, const Vector3 &position, const short &owner)
{
    ...
    if(tWC.hasRoamArea)
    {
        tCritter->setRoamAreaID((unsigned char)tWC.roamAreaID);
    }
...
MagixCritter* MagixUnitManager::createCritter(const unsigned short &iID, const String &type,
    const Vector3 &position, const short &owner)
{
...
void MagixUnitManager::rewardCritter(MagixCritter *critter)
{
    ...
    for(int j = 0; j < (int)tDropList.size(); j++)
    {
        if(Math::UnitRandom() < tDropList[j].second)
        {
            ...
            if(mGameStateManager->isCampaign())
            {
                createItem(mItemManager->getNextEmptyID(0), tDropList[j].first,
                    Vector2(tPos.x, tPos.z));
            }
            else
            {
                itemDropQueue.push_back(std::pair<String, Vector2>(tDropList[j].first,
                    Vector2(tPos.x, tPos.z)));
            }
        }
    }
    ...
    if(tSkill.first != "")
    {
        unsigned char tStock = tSkill.second > 1 ? (unsigned char)Math::RangeRandom((Real)(
            tSkill.second / 2), (Real)(tSkill.second)) : 1;
...
{
    for(vector<std::pair<OwnerToken, String>>::type::iterator it = partyMembers.begin();
        it != partyMembers.end(); it++)
    {
...
bool MagixUnitManager::isPartyMember(MagixUnit *unit)
{
    ...
    {
        if(partyMembers[i].first == tToken)
        {
            return true;
        }
    }
...
const std::pair<OwnerToken, String> MagixUnitManager::getPartyMember(
    const OwnerToken &token)
{
    for(int i = 0; i < (int)partyMembers.size(); i++)
    {
        if(partyMembers[i].first == token)
        {
