...
void MagixAlertBox::initialize()
{
    ...
    mAlertBoxText = OverlayManager::getSingleton().getOverlayElement(
        "GUI/AlertBoxText");
}
...
void MagixAlertBox::update(const FrameEvent evt)
{
    ...
    if(alertTimeout > 0)
    {
        ...
        mAlertBox->setPosition(startLeft + (1 - tRatio)*startWidth / 2,
            startTop + (1 - tRatio)*startHeight / 2);
...
void MagixAlertBox::showAlert(const String &caption, Real left, Real top,
    Real count)
{
    ...
    startHeight = StringConverter::parseReal(
        mAlertBoxText->getParameter("char_height"));
