...
void MagixPlayer::updateMovement(const FrameEvent &evt, const Quaternion &camOrientation)
{
    ...
    else if(controlMode == CONTROL_ABSOLUTE || controlMode == CONTROL_FIRSTPERSON)
    {
        if(up || down || left || right || ascend || descend)
        {
            ...
            if(left)
            {
                addForce(Vector3(tVect.perpendicular().x, 0,

                    tVect.perpendicular().y)*-10 * evt.timeSinceLastFrame);
                if(controlMode == CONTROL_FIRSTPERSON)
                {
                    setYaw(Degree(90) + tYaw + (up ? Degree(-45) : Degree(0)) +
                        (down ? Degree(225) : Degree(0)));
                }
                else
                {
                    setYaw(Degree(90) + tYaw + (up ? Degree(-45) : Degree(0)) +
                        (down ? Degree(45) : Degree(0)));
                }
            }
            else if(right)
            {
                ...
                if(controlMode == CONTROL_FIRSTPERSON)
                {
                    setYaw(Degree(270) + tYaw + (up ? Degree(45) : Degree(0)) +
                        (down ? Degree(-225) : Degree(0)));
                }
                else
                {
...
void MagixPlayer::updateAutoAttack(MagixExternalDefinitions *def)
{
    ...
    if(getPosition().squaredDistance(mAutoAttackTarget->getPosition()) <
        (AUTOTARGET_RANGE_SQUARED * 0.25 + nextAttackRange * nextAttackRange))
    {
        if(!mNextSkill)
        {
            doAttack(def);
...
void MagixPlayer::update(const FrameEvent &evt, MagixExternalDefinitions *def,
    const Quaternion &camOrientation)
{
    if(!unitID)
    {
        return;
