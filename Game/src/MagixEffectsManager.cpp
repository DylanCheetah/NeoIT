/*
Notes:
Special effects are drawn on planes called billboards.
*/
...
void MagixEffectsManager::reset(bool ignoreUnresettable)
{
    ...
    {
        destroyRibbonTrail(i);
    }

    for(int i = 0; i < MAX_PARTICLES; i++)
    {
        if(ignoreUnresettable || !particleUnresettable[i])
        {
            destroyParticle(i);
        }
    }
    ...
    {
        destroyItemParticle(i);
    }

    if(ignoreUnresettable)
    {
...
void MagixEffectsManager::update(const FrameEvent &evt)
{
    for(int i = 0; i < MAX_PARTICLES; i++)
    {
        if(mParticle[i])
        {
            ...
            else if(particleDuration[i] == 0 &&
                mParticle[i]->getNumParticles() == 0)
            {
                destroyParticle(i);
            }
        }
    ...
    for(int i = 0; i < MAX_WATERRIPPLES; i++)
    {
        if(mWaterRipple[i])
        {
            ...
            {
                mWaterRippleNode[i]->setPosition(
                    mWaterRippleOwner[i]->getPosition().x,
                    mWaterRippleNode[i]->getPosition().y,
                    mWaterRippleOwner[i]->getPosition().z);
            }

            //Destroy afterall all particles are gone
            if(mWaterRipple[i]->getEmitter(0)->getEmissionRate() == 0 &&
                mWaterRipple[i]->getNumParticles() == 0)
            {
                destroyWaterRipple(i);
            }
        }
    ...
    for(int i = 0; i < MAX_BREATHERS; i++)
    {
        ...
        {
            SceneNode* tBreatherNode = mSceneMgr->getSceneNode("BreatherNode" +
            ...
            tBreatherNode->setPosition(tBone->_getDerivedPosition() +
                tBone->_getDerivedOrientation() * breatherOffset[i]);
        }
    }
    ...
    for(int i = 0; i < MAX_ITEMEFFECTS; i++)
    {
        ...
        {
            SceneNode* tItemEffectNode = mSceneMgr->getSceneNode("ItemFXNode" +
                StringConverter::toString(i));
            Bone *tBone = mItemEffectOwner[i]->getSkeleton()->getBone(
                itemEffectBone[i]);
            tItemEffectNode->setOrientation(tBone->_getDerivedOrientation());
            tItemEffectNode->setPosition(tBone->_getDerivedPosition() +
...
void MagixEffectsManager::createRibbonTrail(Entity *ent, const String &boneName,
    const String &trailMat, const Real &trailWidth, const ColourValue &trailColour,
    const ColourValue &colourChange, const String &headMat)
{
    short tID = -1;
    ...
    mTrailHead[tID] = mSceneMgr->createBillboardSet("FXTrailHead" +
        StringConverter::toString(tID));
    ...
    TagPoint *tTP = mTrailHeadEnt[tID]->attachObjectToBone(boneName,
        mTrailHead[tID]);

    if(headMat == "")
    {
        mTrailHead[tID]->setVisible(false);
    }

    mTrail[tID] = mSceneMgr->createRibbonTrail("FXTrail" +
        StringConverter::toString(tID));
...
{
    if(mTrailHead[tID] && mSceneMgr->hasBillboardSet("FXTrailHead" +
        StringConverter::toString(tID)))
    {
    ...
    if(mTrail[tID] && mSceneMgr->hasRibbonTrail("FXTrail" +
        StringConverter::toString(tID)))
    {
...
ParticleSystem* MagixEffectsManager::createParticle(SceneNode *node,
    const String &name, const Real &duration, bool unresettable,
    bool attachToBone, bool hasOwnNode, const Vector3 &position,
    const String &ownName)
{
    ...
    mParticle[tID] = mSceneMgr->createParticleSystem(
        ownName == "" ? "FXParticle" + StringConverter::toString(tID) : ownName,
        name);
    mParticleNode[tID] = (hasOwnNode ? mSceneMgr->getRootSceneNode()->createChildSceneNode(
        "FXParticleNode" + StringConverter::toString(tID)) : node);

    if(hasOwnNode)
    {
        mParticleNode[tID]->setPosition(position);
    }

    if(!attachToBone)
    {
        node->attachObject(mParticle[tID]);
    }
...
void MagixEffectsManager::destroyParticle(unsigned short tID)
{
    if(mParticle[tID]/* && mSceneMgr->hasParticleSystem("FXParticle"+StringConverter::toString(tID))*/)
    {
        ...
        if(particleHasOwnNode[tID])
        {
            mSceneMgr->destroySceneNode(mParticleNode[tID]);
        }
...
void MagixEffectsManager::destroyParticle(const String &name)
{
    for(int i = 0; i < MAX_PARTICLES; i++)
    {
        if(mParticle[i] && mParticle[i]->getName() == name)
        {
            destroyParticle(i);
            return;
        }
...
void MagixEffectsManager::destroyParticleByObjectNode(SceneNode *node,
    const String &type)
{
    for(int i = 0; i < MAX_PARTICLES; i++)
    {
...
void MagixEffectsManager::stopParticleByObjectNode(SceneNode *node,
    const String &type)
{
    for(int i = 0; i < MAX_PARTICLES; i++)
    {
...
void MagixEffectsManager::createWaterRipple(SceneNode *owner,
    const Real &waterHeight, const Real &scale)
{
    short tID = -1;
    ...
    mWaterRipple[tID] = mSceneMgr->createParticleSystem("FXWaterRipple" +
        StringConverter::toString(tID), "WaterRipple");
    mWaterRipple[tID]->setDefaultDimensions(scale, scale);
    mWaterRippleNode[tID] = mSceneMgr->getRootSceneNode()->createChildSceneNode(
        "WaterRippleNode" + StringConverter::toString(tID));
    mWaterRippleNode[tID]->attachObject(mWaterRipple[tID]);
    mWaterRippleOwner[tID] = owner;
    mWaterRippleNode[tID]->setPosition(owner->getPosition().x, waterHeight,
        owner->getPosition().z);
}
...
{
    if(mWaterRipple[tID] && mSceneMgr->hasParticleSystem("FXWaterRipple" +
        StringConverter::toString(tID)))
    {
        mWaterRippleNode[tID]->detachObject(mWaterRipple[tID]);
        mSceneMgr->destroyParticleSystem(mWaterRipple[tID]);
        mSceneMgr->destroySceneNode("WaterRippleNode" +
            StringConverter::toString(tID));
        mWaterRipple[tID] = 0;
...
void MagixEffectsManager::setWaterRippleEmissionByOwner(SceneNode *node,
    const Real &rate)
{
    for(int i = 0; i < MAX_WATERRIPPLES; i++)
    {
...
void MagixEffectsManager::setWaterRippleHeightByOwner(SceneNode *node,
    const Real &height)
{
    for(int i = 0; i < MAX_WATERRIPPLES; i++)
    {
        if(mWaterRipple[i] && mWaterRippleOwner[i] == node)
        {
            mWaterRippleNode[i]->setPosition(node->getPosition().x, height,
                node->getPosition().z);
            return;
        }
    }
...
void MagixEffectsManager::createBreather(Entity *owner, const String &type,
    const Vector3 &offset, const Real &scale)
{
    short tID = -1;
    ...
    mBreather[tID] = mSceneMgr->createParticleSystem("FXBreather" +
        StringConverter::toString(tID), type);
    ...
    SceneNode* tBreatherNode = owner->getParentSceneNode()->createChildSceneNode(
        "BreatherNode" + StringConverter::toString(tID));
...
{
    if(mBreather[tID] && mSceneMgr->hasParticleSystem("FXBreather" +
        StringConverter::toString(tID)))
    {
        SceneNode* tBreatherNode = mSceneMgr->getSceneNode("BreatherNode" +
            StringConverter::toString(tID));
        ...
        mSceneMgr->destroySceneNode("BreatherNode" +
            StringConverter::toString(tID));
        mBreather[tID] = 0;
...
void MagixEffectsManager::destroyBreatherByOwner(Entity *ent, const String &type)
{
    for(int i = 0; i < MAX_BREATHERS; i++)
    {
        if(mBreatherOwner[i] == ent && (type == "" || breatherType[i] == type))
        {
            destroyBreather(i);
        }
    }
...
void MagixEffectsManager::setBreatherEmissionByOwner(Entity *ent,
    const Real &rate, const String &type)
{
    for(int i = 0; i < MAX_BREATHERS; i++)
    {
        if(mBreather[i] && mBreatherOwner[i] == ent &&
            (type == "" || breatherType[i] == type))
        {
...
void MagixEffectsManager::createItemParticle(SceneNode *node)
{
    ...
    mItemParticle[tID] = mSceneMgr->createParticleSystem("ItemParticle" +
        StringConverter::toString(tID), "item");
...
{
    if(mItemParticle[tID] && mSceneMgr->hasParticleSystem("ItemParticle" +
        StringConverter::toString(tID)))
    {
...
void MagixEffectsManager::createItemEffect(Entity *owner, const String &type,
    const String &bone, const Vector3 &offset, const Real &scale)
{
    short tID = -1;
    ...
    mItemEffect[tID] = mSceneMgr->createParticleSystem("ItemFX" +
        StringConverter::toString(tID), type);
    mItemEffect[tID]->setDefaultDimensions(mItemEffect[tID]->getDefaultWidth() *
        scale, mItemEffect[tID]->getDefaultHeight() * scale);
    SceneNode* tItemEffectNode = owner->getParentSceneNode()->createChildSceneNode(
        "ItemFXNode" + StringConverter::toString(tID));
    ...
    if(bone == "")
    {
        tItemEffectNode->setPosition(offset);
    }
}
...
{
    if(mItemEffect[tID] && mSceneMgr->hasParticleSystem("ItemFX" +
        StringConverter::toString(tID)))
    {
        SceneNode* tItemEffectNode = mSceneMgr->getSceneNode("ItemFXNode" +
            StringConverter::toString(tID));
...
void MagixEffectsManager::destroyItemEffectByOwnerNode(SceneNode *node,
    const String &type)
{
    for(int i = 0; i < MAX_ITEMEFFECTS; i++)
    {
        ...
        {
            if(mItemEffectOwner[i]->getParentSceneNode() == node &&
                (type == "" || itemEffectType[i] == type))
            {
                destroyItemEffect(i);
            }
        }
