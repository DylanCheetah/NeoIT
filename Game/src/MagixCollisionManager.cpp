/*
Notes:
This file contains the code for managing portals, gates, and water boxes.
*/
...
void MagixCollisionManager::createSphereMesh(const std::string& strName,
...
void MagixCollisionManager::createCollision(SceneNode *node, const Real &range,
    const Vector3 &force, const Vector3 &offset, const short &hp,
    const unsigned char &alliance, bool hitAlly)
{
    Collision tColl;
    tColl.mNode = node;
    tColl.offset = offset;
...
const list<Collision>::type::iterator MagixCollisionManager::destroyCollision(
    const list<Collision>::type::iterator &it)
{
...
const vector<Collision*>::type MagixCollisionManager::getCollisionHitList(
    const unsigned short &unitID, const unsigned short &alliance,
    const AxisAlignedBox &target)
{
    ...
    while(it != coll.end())
    {
        ...
        if((tColl->hitAlly && tColl->alliance == alliance) || (!tColl->hitAlly &&
            tColl->alliance != alliance))
        {
...
const vector<Collision*>::type MagixCollisionManager::getCollisionHitListForCritter(
    const unsigned short &iID, const unsigned short &alliance,
    const AxisAlignedBox &target)
{
    vector<Collision*>::type tList;
    list<Collision>::type::iterator it = coll.begin();
...
void MagixCollisionManager::createBoxWall(const Vector3 &center,
    const Vector3 &range, bool isInside)
{
    Wall tWall;
    tWall.isActive = true;
...
void MagixCollisionManager::createSphereWall(const Vector3 &center,
    const Real &range, bool isInside)
{
    Wall tWall;
    tWall.isActive = true;
    tWall.isSphere = true;
...
const list<Wall>::type::iterator MagixCollisionManager::destroyWall(
    const list<Wall>::type::iterator &it)
{
...
const vector<Wall*>::type MagixCollisionManager::getWallHitList(
    const Vector3 &target)
{
    ...
    while(it != wall.end())
    {
        ...
        if(tWall->collides(target))
        {
            tList.push_back(tWall);
        }
...
void MagixCollisionManager::createPortal(const Vector3 &center, const Real &range,
    String destName)
{
    if(!mSceneMgr)
    {
    ...
    Portal tPortal;
    tPortal.mEnt = mSceneMgr->createEntity("PortalSphere" +
        StringConverter::toString(portal.size() + 1), "portal.mesh");
...
void MagixCollisionManager::getPortalMap(
    vector<std::pair<Vector2, String>>::type &map)
{
    map.clear();
    list<Portal>::type::iterator it = portal.begin();
    ...
    while(it != portal.end())
    {
        ...
        if(tPortal->mNode)
        {
            const std::pair<Vector2, String> tData = std::pair<Vector2, String>(
                Vector2(tPortal->mNode->getPosition().x,
                tPortal->mNode->getPosition().z), tPortal->dest);
            map.push_back(tData);
        }
...
void MagixCollisionManager::createWaterBox(const Vector3 &center,
    const Real &scaleX, const Real &scaleZ, bool isSolid)
{
    WaterBox tBox;
    tBox.isActive = true;
    tBox.center = center;
...
const list<WaterBox>::type::iterator MagixCollisionManager::destroyWaterBox(
    const list<WaterBox>::type::iterator &it)
{
...
void MagixCollisionManager::createGate(const Vector3 &center, String destName,
    const Vector3 &destVect, const String &matName, bool hasVectY)
{
    if(!mSceneMgr)
    {
        return;
    ...
    Gate tGate;
    tGate.mEnt = mSceneMgr->createEntity("GateSphere" +
        StringConverter::toString(gate.size() + 1), "portal.mesh");
...
const list<Gate>::type::iterator MagixCollisionManager::destroyGate(
    list<Gate>::type::iterator it)
{
    if(!mSceneMgr)
    {
        return ++it;
...
void MagixCollisionManager::getGateMap(
    vector<std::pair<Vector2, String>>::type &map)
{
    map.clear();
    list<Gate>::type::iterator it = gate.begin();
    ...
    while(it != gate.end())
    {
        ...
        {
            const std::pair<Vector2, String> tData = std::pair<Vector2, String>(
                Vector2(tGate->mNode->getPosition().x,
                tGate->mNode->getPosition().z), tGate->dest);
            map.push_back(tData);
        }
...
const vector<CollBox*>::type MagixCollisionManager::getCollBoxHitList(
    const Vector3 &target, const Real &headHeight)
{
    ...
    while(it != collBox.end())
    {
        ...
        if(tBox->collides(target, headHeight))
        {
            tList.push_back(tBox);
        }
...
void MagixCollisionManager::createCollSphere(const Vector3 &center,
    const Real &range)
{
    CollSphere tSphere;
...
const vector<CollSphere*>::type MagixCollisionManager::getCollSphereHitList(
    const Vector3 &target)
{
    ...
    while(it != collSphere.end())
    {
        ...
        if(tSphere->collides(target))
        {
            tList.push_back(tSphere);
        }
