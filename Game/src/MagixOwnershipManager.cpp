...
//Ownership Manager Functions
//==============================================================
MagixOwnershipManager::MagixOwnershipManager()
{
    mDef = 0;
    mGameStateManager = 0;
    mCritterManager = 0;
...
void MagixOwnershipManager::initialize(MagixExternalDefinitions *def,
    MagixGameStateManager *gameStateMgr, MagixCritterManager *critterMgr,
    MagixUnitManager *unitMgr, MagixNetworkManager *networkMgr, MagixWorld *world,
    MagixChatManager *chatMgr, MagixSkyManager *skyMgr)
{
    mDef = def;
...
void MagixOwnershipManager::update(const FrameEvent &evt)
{
...
void MagixOwnershipManager::updateCritterOwnership(const FrameEvent &evt)
{
    ...
    for(int i = 0; i < (int)tAbandonList.size(); i++)
    {
        ...
        if(tSuiter)
        {
            mNetworkManager->sendCritterTakeover(tC->getID(), (OwnerToken)tSuiter->getIndex());
        }
    }
}


void MagixOwnershipManager::updateCritters(const FrameEvent &evt)
{
    if(spawnTimer > 0)
    {
        spawnTimer -= evt.timeSinceLastFrame;
    }
    ...
    if(spawnTimer <= 0 && mCritterManager->getNumCritters() < mWorld->getCritterSpawnLimit())
    {
        ...
        if(tList.size() > 0)
        {
            ...
            if(Math::UnitRandom() < (tCritter.spawnRate))
            {
                ...
                {
                    const std::pair<Vector3, Vector3> tRoamArea = mWorld->getCritterRoamArea(
                        tCritter.roamAreaID);
                    tX = Math::RangeRandom(
                        tRoamArea.first.x, tRoamArea.first.x + tRoamArea.second.x);
                    ...
                    tZ = Math::RangeRandom(tRoamArea.first.z,
                        tRoamArea.first.z + tRoamArea.second.z);
                ...
                if(mGameStateManager->isCampaign())
                {
                    mUnitManager->createCritter(mCritterManager->getNextEmptyID(0),
                        (unsigned char)tID, Vector3(tX, tY, tZ));
                }
                else
                {
                    mNetworkManager->sendCritterCreate((unsigned char)tID, Vector3(tX, tY, tZ));
                }
            }
        }
    }
    ...
    for(int i = 0; i < (int)tDList.size(); i++)
    {
        mUnitManager->deleteCritter(tDList[i]->getID());
    }
    ...
    if(stationaryCritterTimer <= 0)
    {
        ...
        for(int i = 0; i < (int)tCritterStationaryList.size(); i++)
        {
            ...
            //From player
            const Real tSquaredDist = tC->getPosition().squaredDistance(
                mUnitManager->getPlayer()->getPosition());

            if(tSquaredDist < 250000 &&
                (!mUnitManager->getPlayer()->getIsCrouching() ||
                mUnitManager->getPlayer()->isAttacking() ||
                mUnitManager->getPlayer()->isAntiGravity()) && Math::UnitRandom() < 0.05)
            {
                if(tC->getDecisionTimer() > 1)
                {
                    tC->setDecisionTimer(1);
                }
            ...
            for(int j = 0; j < (int)tUnitList.size(); j++)
            {
                ...
                if(tSquaredDist2 < 250000 && (!tUnit->getIsCrouching() ||
                    tUnit->isAttacking() || tUnit->isAntiGravity()) && Math::UnitRandom() < 0.05)
                {
                    if(tC->getDecisionTimer() > 1)
                    {
                        tC->setDecisionTimer(1);
    ...
    for(int i = 0; i < (int)tCritterDecisionList.size(); i++)
    {
        ...
        if(tC->getIsPet() && mUnitManager->getPlayer()->getPet() == tC->getCritterType())
        {
            ...
            if(!tPetFlags->stay)
            {
                ...
                {
                    const bool tFriendlyAttack = (tPetFlags->attackTarget->matchAlliance(
                        tC->getAlliance()));
                    const unsigned char tAttackID = mDef->getCritterRandomSpecificAttack(
                        tC->getCritterType(), !tFriendlyAttack);
                    ...
                    if(tAttackID != 0)
                    {
                        ...
                        if(!mGameStateManager->isCampaign())
                        {
                            ...
                            {
                                mNetworkManager->sendCritterAttackDecision(tC->getID(),
                                    static_cast<MagixCritter*>(tPetFlags->attackTarget)->getID(),
                                    tAttackID, true);
                            }
                            else if(tPetFlags->attackTarget->getType() == OBJECT_UNIT && static_cast<MagixUnit*>(tPetFlags->attackTarget)->isIndexedUnit())
                            {
                                mNetworkManager->sendCritterAttackDecision(tC->getID(),
                                    static_cast<MagixIndexedUnit*>(tPetFlags->attackTarget)->getIndex(),
                                    tAttackID);
                            }
                            else if(tPetFlags->attackTarget == mUnitManager->getPlayer())
                            {
                                mNetworkManager->sendCritterAttackDecision(tC->getID(),
                                    mUnitManager->getPlayer()->getIndex(), tAttackID);
                            }
                        }
                    }
                }
                else
                {
                    ...
                    Real tDist = (Real)(tC->getLength()*0.8);
                    ...
                    {
                        tC->setTarget(tPlayerPos + Vector3(Math::RangeRandom(-tDist, tDist),
                            tC->isAntiGravity() ? tHeight : 0, Math::RangeRandom(-tDist, tDist)));
        ...
        else
        {
            ...
            if(tLastHitIndex == 0)
            {
                ...
                {
                    tTargetUnit = mUnitManager->getPlayer();
                }
            }
            ...
            {
                bool tHitAlly = false;
                const unsigned char tAttackID = mDef->getCritterRandomAttack(tC->getCritterType(),
                    &tHitAlly);
                ...
                if(tHitAlly)
                {
                    ...
                    if(!mGameStateManager->isCampaign())
                    {
                        mNetworkManager->sendCritterAttackDecision(tC->getID(), tC->getID(),
                            tAttackID, true);
                    }
                }
                //Attack enemy
                ...
                {
                    const OwnerToken tTargetID = tLastHitIndex == 0 ?
                        mUnitManager->getPlayer()->getIndex() : tLastHitIndex;
                    ...
                    if(!mGameStateManager->isCampaign())
                    {
                        mNetworkManager->sendCritterAttackDecision(tC->getID(), tTargetID, tAttackID);
                    }
                }
            }
            //Roam about
            else
            {
                if(tC->getRoamArea())
                {
                    const std::pair<Vector3, Vector3> tRoamArea = mWorld->getCritterRoamArea(
                        tC->getRoamAreaID());
                    const Real tX = Math::RangeRandom(tRoamArea.first.x,
                        tRoamArea.first.x + tRoamArea.second.x);
                    const Real tY = Math::RangeRandom(tRoamArea.first.y,
                        tRoamArea.first.y + tRoamArea.second.y);
                    const Real tZ = Math::RangeRandom(tRoamArea.first.z,
                        tRoamArea.first.z + tRoamArea.second.z);
                    tC->setTarget(Vector3(tX, tY, tZ));
                }
                else
                {
                    tC->setTarget(tC->getPosition() + Vector3(Math::RangeRandom(-300, 300),
                        tC->isAntiGravity() ? Math::RangeRandom(-50, 50) : 0, Math::RangeRandom(-300, 300)));
                }
            ...
            tC->setDecisionTimer(Math::RangeRandom(tInfo.decisionMin,
                (Real)(tInfo.decisionMin + tInfo.decisionDeviation)));
        }
    }
    ...
    if(mUnitManager->getPlayer()->getPet() != "" && !tPetFlags->attackTarget &&
        tPetFlags->hasHeal && mUnitManager->getPlayer()->getHPRatio() < 1 &&
        !mUnitManager->getPlayer()->getIsWounded() && Math::UnitRandom() < 0.1)
    {
    ...
    if(tCritter)
    {
        ...
        if(!tCritter->matchAlliance(mUnitManager->getPlayer()->getAlliance()) &&
            mUnitManager->getPlayer()->getPet() != "" && !tPetFlags->attackTarget)
        {
    ...
    if(mUnitManager->getPlayerTarget() &&
        mUnitManager->getPlayerTarget()->getType() == OBJECT_CRITTER)
    {
    ...
    if(mUnitManager->getPlayer()->getIsLockedOn() &&
        mUnitManager->getPlayer()->getAutoTrackObject() &&
        mUnitManager->getPlayer()->getAutoTrackObject()->getType() == OBJECT_CRITTER)
    {
        if(static_cast<MagixCritter*>(mUnitManager->getPlayer()->getAutoTrackObject())->getIsDead())
        {
            mUnitManager->getPlayer()->lockOn(0);
        }
    }
    ...
    if(!mGameStateManager->isCampaign())
    {
        ...
        for(int i = 0; i < (int)tCritterHitList.size(); i++)
        {
            ...
            if(!tCritterHitList[i].isMine)
            {
                mNetworkManager->sendCritterTakeover(tCritterHitList[i].ID);
            }
        }
...
void MagixOwnershipManager::updateUnits(const FrameEvent &evt)
{
    //HitInfo
    if(!mGameStateManager->isCampaign())
    {
        ...
        {
            mNetworkManager->sendPlayerHit((OwnerToken)(tUnitHitList[i].ID), tUnitHitList[i].hp,
                tUnitHitList[i].force);
        }
    }
}
