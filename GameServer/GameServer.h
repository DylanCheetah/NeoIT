...
public:
	...
	void begin();
	void runLoop();
	void printChat(Packet *p, RakNet::BitStream *bitStream,
		const char* pmTarget = "");
	void printChat(const OwnerToken &sender, RakNet::BitStream *bitStream,
		const char* pmTarget = "");
	void sendNewPlayerProcedure(const OwnerToken &playerToken,
		const string &mapName, const unsigned char &dimension);
	void broadcastInMap(const string &mapName, const unsigned char &dimension,
		RakPeerInterface *peer, RakNet::BitStream *bitStream,
	...
	void sendMyPlayers(RakPeerInterface *peer, RakNet::BitStream *bitStream,
		const PacketPriority &priority, const PacketReliability &reliability,
		const char &orderingChannel, const SystemAddress &exceptionAddress);
	void relayPacket(Packet *p, const PacketPriority &priority,
		const PacketReliability &reliability, const char &orderingChannel);
	void sendAllServers(RakNet::BitStream *bitStream,
		const PacketPriority &priority, const PacketReliability &reliability,
		const char &orderingChannel, const SystemAddress &exceptionAddress);
	void sendNextServer(RakNet::BitStream *bitStream,
		const PacketPriority &priority, const PacketReliability &reliability,
	...
	const OwnerToken getTokenByName(const string &playerName,
	...
	const unsigned short playersInMap(const string &mapName,
		const unsigned char &dimension);
	void assignPlayerData(const OwnerToken &token, bool isMine, Packet *p,
		const string &playerName, const string &mapName,
		const unsigned char &dimension/*, const unsigned short &charID*/);
	...
	void sendDisconnectProcedureToMyPlayers(const OwnerToken &token,
	...
	void sendDisconnectProcedureOfAllTokensConnectedTo(
